...
- Late same-day waves covering Signal K SSRF, PHP JWT/JWE algorithm or padding boundaries, OTP provisioning URI mass assignment, Gemini MCP CLI command construction, and OpenClaw chat/env/exec/recovery controls are promotable when they expose reusable URL-connector, token-parser, enrollment, MCP-wrapper, or agent policy-state boundaries. Keep proofs to owned callbacks, disposable keys/tokens, synthetic OTP URIs, fake `@file` targets, temp PATH/env canaries, lab chat identities, and marker-only config/memory evidence.
