...
- File Browser/Fleet/Fabric.js/Cordova waves are promotable when they expose reusable future-grant, prefix-cleanup, cursor-oracle, serialization, or mobile bridge boundaries: pre-created shares to later files, path-prefix share deletion across users, `ORDER BY` plus cursor pagination leaking joined-table canaries, Fabric canvas data crossing into trusted SVG DOM insertion, or InAppBrowser web content dispatching predictable native callback IDs. Keep proofs to disposable files/shares, seeded Fleet canaries, harmless SVG markers, and test Cordova callbacks.
