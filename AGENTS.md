...
- File Browser/Fleet/Fabric.js/Cordova waves are promotable when they expose reusable future-grant, prefix-cleanup, public-share rule-rebase, archive separator traversal, scoped-root symlink, cursor-oracle, serialization, or mobile bridge boundaries: pre-created shares to later files, path-prefix share deletion across users, public directory shares bypassing descendant deny rules, zip/tar entries preserving Windows traversal separators, in-scope symlinks reaching out-of-scope canaries, `ORDER BY` plus cursor pagination leaking joined-table canaries, Fabric canvas data crossing into trusted SVG DOM insertion, or InAppBrowser web content dispatching predictable native callback IDs. Keep proofs to disposable files/shares, deny-rule markers, archive listings, symlink canaries, seeded Fleet canaries, harmless SVG markers, and test Cordova callbacks; skip pure public-login DoS unless paired with an operator workflow.
