...
| [GHSA-2xgm-wc4g-5jvg](https://github.com/advisories/GHSA-2xgm-wc4g-5jvg) | Workflow creation trusted a supplied folder reference without binding it to the caller's project, letting an authenticated user assign a workflow into another project's folder before n8n 2.28.0. |

Build an identity matrix with two synthetic issuers that deliberately reuse one subject, verified and unverified canary emails, low/high role claims, and a disposable local account per expected identity. Exercise only harmless profile/status and marker-only Public API routes. Separately create two disposable projects with one folder each and test whether project A can create a canary workflow while naming project B's folder; record workflow owner/project and actual folder placement. Capture issuer, subject, email-verification state, trusted-key role ceiling, provisioned local identity, resulting API scopes, object IDs, caller membership, and fixed-version result. Never impersonate real users, create production admins, install packages, delete users, alter real folders, or reuse live tokens.
...
| [GHSA-jp7m-xcgx-57qm](https://github.com/advisories/GHSA-jp7m-xcgx-57qm) / CVE-2026-59259 | Static credential validation and the runtime expression parser disagreed about external-secret reference forms, letting a credential editor without `externalSecret:list` resolve a configured provider's secret during execution. |
| [GHSA-2434-3x6q-8r99](https://github.com/advisories/GHSA-2434-3x6q-8r99) / CVE-2026-59254 | Ordinary workflow node expressions resolved external secrets outside the credential context, exposing the plaintext canary to a project editor without secret permissions. |

Use two disposable editors, a shared fake credential containing a unique inert header, two owned callback origins, and a lab external-secret provider containing only a synthetic canary. Compare direct HTTP Request, pagination, AI/LLM endpoint override, MCP connector, resolved versus expression-backed `genericAuthType`, inline versus referenced sub-workflows, and normal versus parser-variant external-secret references. A positive proof is the fake header arriving at the wrong owned origin, an unauthorized credential reference being accepted, or the synthetic provider canary resolving without `externalSecret:list`. Do not recover production credential values, target third-party APIs, or use a live secret provider.
...
| [GHSA-hwmj-qg4v-cvg9](https://github.com/advisories/GHSA-hwmj-qg4v-cvg9) / CVE-2026-59257 | MySQL v1 `executeQuery` interpolated evaluated expression values into raw SQL; an externally triggered workflow could carry caller input into query structure. MySQL v2 is not affected. |

For expressions and Git hooks, use an inert temp-file marker in an isolated n8n container; host-shell access or persistence is unnecessary. For URL paths, use an owned callback and a synthetic loopback service created for the test—never metadata or real internal services. For Git and file checks, place unique marker repositories/files immediately outside disposable roots and prove only the path crossing. For Send Email, use fake SMTP plus synthetic local/URL content; for Edit Image, write only a disposable marker; for `search_files`, return only a known adjacent canary file. For MySQL v1, use a scratch schema with seeded canary rows and compare a bound-value control, expression interpolation, MySQL v2, and fixed versions; stop at a changed synthetic result set and never query production or destructive statements.
...
## Final July 22 n8n MCP, GraphQL, computer-use, and LLM credential follow-up

Four unique advisories extend the same workflow-authority matrix; duplicate GHSA records were tracked without duplicating guidance. The common fixes are n8n 1.123.64, 2.29.8, and 2.30.1 where those release lines apply.

| Advisory | Boundary to validate |
| --- | --- |
| [GHSA-q5xf-xhwf-cwqf](https://github.com/advisories/GHSA-q5xf-xhwf-cwqf) / CVE-2026-65594 | From n8n 2.27.0, a member could self-approve OAuth consent for another user's `n8n OAuth2` MCP Server Trigger, invoke its tools under the owner's project/credentials, and read tool output. |
| [GHSA-gq66-9cw5-j5jm](https://github.com/advisories/GHSA-gq66-9cw5-j5jm) / CVE-2026-65596 | GraphQL nodes did not enforce a shared HTTP credential's Allowed HTTP Request Domains, unlike HTTP Request nodes. |
| [GHSA-fpg6-x68q-5793](https://github.com/advisories/GHSA-fpg6-x68q-5793) / CVE-2026-65590 | Optional `@n8n/computer-use` shell restrictions were applied only on macOS; Linux and Windows ran the tool without the promised filesystem/network sandbox. Standard n8n installations without this package are not affected. |
| [GHSA-89gh-3pgc-v5h2](https://github.com/advisories/GHSA-89gh-3pgc-v5h2) / CVE-2026-65589 | Custom HTTP header values for certain LLM sub-node credentials were masked in the UI but persisted in plaintext execution data visible to users with workflow execution-data access. |

Use two disposable projects and member/owner users, one marker-only MCP tool, two owned callback origins, a fake shared header credential, a no-permission fake LLM key, and an isolated computer-use worker. For MCP OAuth, compare resource ownership at client registration, consent, token issuance, tool listing, and invocation; stop after the inert marker. For GraphQL, send the fake header to allowed and disallowed owned origins and compare with HTTP Request. For computer-use, run only a temp-file and synthetic-loopback reachability matrix on each OS. For LLM execution data, run one synthetic workflow and inspect only whether the unique fake header marker appears to an authorized lab viewer; preserve a hash or redacted shape rather than the value.

Report **member identity to another owner's MCP workflow**, **shared credential to disallowed GraphQL destination**, **documented sandbox to unrestricted worker marker**, or **masked credential to persisted execution artifact**. Never mount real home directories, enable production shell tools, use production connectors, or retain credential values. The unauthenticated test-webhook cancellation item was tracked but not promoted because its impact is limited to transient test-session availability.

Three later entries add SQL and cross-user runner-state checks, fixed in n8n 1.123.67, 2.31.5, and 2.32.1:

- [GHSA-652q-gvq3-74qv](https://github.com/advisories/GHSA-652q-gvq3-74qv): Snowflake `executeQuery` interpolated externally controlled expression values into raw SQL instead of positional parameters.
- [GHSA-jqwr-vx3p-r266](https://github.com/advisories/GHSA-jqwr-vx3p-r266): Postgres Trigger channel, function, and trigger identifiers crossed into SQL without identifier-safe construction.
- [GHSA-9cmh-xcqm-5hqr](https://github.com/advisories/GHSA-9cmh-xcqm-5hqr): the JavaScript task runner shared one built-in/external module cache across users, letting one Code-node author alter another user's later module behavior without escaping the sandbox.

For SQL checks, use scratch Snowflake/PostgreSQL schemas with seeded marker rows. Compare scalar placeholders, expression interpolation, identifier fields, quoting variants, and patched query-parameter behavior; prove only a changed synthetic result set or harmless canary object. For the module cache, use two lab users, a deterministic barrier, and a benign local module whose exported marker can be changed without filesystem, network, or process access. Run user A, then user B on the same and separate runners, reverse order, and compare modules disabled and patched releases. Report **external trigger value to Snowflake query structure**, **trigger identifier to PostgreSQL statement structure**, or **user A module cache mutation to user B Code-node result**—not database compromise, sandbox escape, or host RCE.

## September 16 n8n Instance AI, Elasticsearch REST-path, and workflow-disclosure follow-up

The published feed added four advisories (fixed in n8n 2.37.7 / 2.38.2, plus 1.123.76 on the LTS line) that extend the workflow-authority matrix into the Instance AI module, REST-path identifier handling, and the push channel:

| Advisory | Boundary worth testing |
| --- | --- |
| [GHSA-q5wm-mgqx-fv2f](https://github.com/advisories/GHSA-q5wm-mgqx-fv2f) | Instance AI credential setup accepted a credential test/probe URL taken from fetched content without checking it matched the workflow node's origin, so an attacker-influenced URL could receive authenticated setup requests (including redirects). The fix derives the probe destination from the node's own URL and confines requests, redirects, and probes to that origin. |
| [GHSA-f2cp-m7mv-8jpv](https://github.com/advisories/GHSA-f2cp-m7mv-8jpv) | Elasticsearch and ElasticSecurity nodes interpolated user-provided identifiers straight into the REST request path; a value containing `/`, `..`, or dot segments redirected the operation to another index or a cluster-administration endpoint under the **stored Elasticsearch credential**. The fix encodes each identifier as one path segment and rejects values that normalize away. |
| [GHSA-679f-58pq-4v2c](https://github.com/advisories/GHSA-679f-58pq-4v2c) | The Instance AI workflow-structure summary used get-or-create-then-nested-write accumulators keyed by stored node names/connection keys. The reserved-name guard n8n shows in the editor is **client-side only** — POSTing the workflow JSON to the REST API bypasses it — so a `__proto__` key landed writes on `Object.prototype` in the main process, affecting every later request. The fix uses null-prototype accumulators and server-side key validation. |
| [GHSA-65xw-2v52-jhxc](https://github.com/advisories/GHSA-65xw-2v52-jhxc) | `/rest/active-workflows` returned every active workflow ID instance-wide to any member regardless of sharing, and activation/deactivation/publication **push events** were broadcast to every connected websocket client with workflow IDs, version IDs, and activation-error detail. The fix scopes both the listing and the push channel through the sharing service. |

Operator patterns to replay on any low-code/workflow platform:

1. **Credential-setup probe SSRF.** In a disposable instance with a fake third-party credential, drive the Instance AI credential-setup flow and observe which URL receives the authenticated probe: an attacker-influenced field (fetched content, node config) rather than the node's declared origin is the finding. Use only an owned no-content callback as the probe destination; never submit real credentials.
2. **REST-path identifier injection.** Configure an Elasticsearch node against a scratch cluster with two marker indices and a benign admin endpoint. Put `marker-a` in the identifier field as the control, then `marker-a/../../_cluster` and dot-segment variants. Positive evidence is the request reaching a different index/admin path under the stored credential — compare the cluster's access log to the intended path. Stop at path-crossing evidence; never touch production clusters or destructive admin endpoints. This is the HTTP-path sibling of the SQL-identifier cases already on this page: **encode value parameters is not the same as encode path segments**.
3. **Client-side-guard bypass.** For every editor-enforced validation, replay the same object through the REST API directly. Here the workflow-JSON key restriction existed only in the editor UI. For prototype pollution, use an inert marker write and check only whether a later unrelated request in the same process observes the marker; restart the lab process to clear state, and never build toward RCE claims without an independent sink.
