...
- [Realtime view-framework state-binding boundaries: djust WebSocket `update_model` mass-assignment of any public view attribute via client-supplied field name (denylist + opt-in allowlist, default allow-all; CVE-2026-61598), djust DEBUG-gated observability/eval endpoints where the localhost gate was opt-in middleware omitted from the docs (7.4; CVE-2026-61590), djust multi-tenant fail-open on WS/SSE + `csrf_exempt` SSE transport CSRF (late follow-ups; CVE-2026-61595/61593), OpenTelemetry.Resources.Host macOS bare-name `sh`/`ioreg` PATH hijack → local LPE (7.0; CVE-2026-81192)](2026-09-16-djust-websocket-mass-assignment-debug-gated-observability-and-otel-path-hijack-ghsa.md)
