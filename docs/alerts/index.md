...
- [Media caption/subtitle tracks as a stored-XSS carrier: Opencast Paella player appends WebVTT/DFXP cue text with `innerHTML += cue` → a non-admin author's subtitle upload executes in every viewer's session (anonymous + staff) on **default config**, caption file served raw to anonymous clients via `/search/episode.json` (CVE-2026-77615/GHSA-m6c8-jcw2-5r25); auxiliary-asset formats (captions, transcripts, chapters, playlists) are an input channel form fuzzers never touch. + Caddy replacer-layer trio folded onto the July 24 Caddy page: `rewrite` trailing-`?` placeholder **double-expansion** leaks `{env.*}`/`{file.*}` into URL/logs/upstream — the exact `vars_regexp` gadget (CVE-2026-30852) unextended to sibling directives (CVE-2026-77281); case-sensitive `hide` matcher vs case-insensitive APFS/NTFS → `.GIT`/`.ENV` served despite hide rules; `{http.request.body}` placeholder read bypasses `max_size` (3 issues, 2 GHSAs)](2026-09-18-opencast-caption-track-stored-xss-carrier-ghsa.md)
