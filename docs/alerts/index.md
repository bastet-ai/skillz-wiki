...
- [**Auth-method precedence + HTTP-verb allowlist bypass + ACL tie-break drift (Sept 21 15:31Z wave, new page)**: Airflow 3.3.0–3.3.1 request with **cookie AND bearer** executes + audit-logs as the **cookie's** principal (bearer precedence inverted, CVE-2026-82355) — send two principals at once and diff executing-vs-logged identity; logout revokes **only the `_token` cookie** — bearer tokens survive "logout" for full TTL (CVE-2026-86473), replay-after-logout on every credential transport; `/assets/events` has no per-Dag filter and the **count query lacks it too** → `total_entries` is a hidden-Dag existence oracle (CVE-2026-75158). MISP <2.5.47 octet: login() enforces bruteforce-block + email-OTP 2FA + failure-logging **only for POST/PUT** — any other verb skips all three (CVE-2026-94379); read-only API key → single call restores full account permissions (CVE-2026-94381); module-results `save()` without unsetting client-supplied `id` — **adjacent loops unset it, this one forgot** — cross-event report read/overwrite/reparent (CVE-2026-94374); UUID-keyed reparenting, event-level-check-only reference authz, non-XML-content XML import → file-read/SSRF, CSRF-missing attribute edit, galaxy-name sprintf XSS. + Dogtag pki-core **lexicographic ACL tie-break** lets a wildcard permission override the specific literal one (CVE-2026-80110, 8.1); Hatchet OAuth state cleared to `""` not deleted → **empty `?state=` accepted** → login-CSRF ATO (CVE-2026-61687); Apache MINA **fix shipped to 2.2.X only** — "fixed" 2.0.29/2.1.13 artifacts still vulnerable, advisory is the exploit map (CVE-2026-94301, 9.8). Rule: security controls are comparisons/lookups — test ordering, sentinel values, coverage, and tie-breaks (16 promoted GHSAs)](2026-09-21-auth-method-precedence-http-verb-allowlists-and-acl-tiebreak-drift-ghsa.md)

- [**Automation-platform credential-test exfil + FQDN-endpoint metadata SSRF + read-only-role credential views** (Sept 21 Red Hat enrichment wave, new page): AAP Controller Vault plugin `kubernetes_auth()` reads the **controller pod's own SA token** and POSTs it to the URL inside an attacker-authored Vault credential when that credential is *Tested* → credential-create role exfiltrates a token with control-plane-namespace pod CRUD + full secret read incl. DB creds + Django `SECRET_KEY` (9.6, CVE-2026-12564) — every "verify/test credential" action that connects out carrying the **platform's own identity** is an exfil channel; enumerate plugin source for token-file reads + attacker-URL requests. OpenShift Router: EndpointSlice write → **FQDN-type endpoint** resolving to cloud metadata → router proxies instance-credential requests there, **explicitly bypassing the earlier IP-address validation** (CVE-2026-42965) — IP-denylist validators cannot check hostnames; grep validation code for `net.ParseIP`-style parsing as evidence of the missing leg, prove with owned hostnames + denied canaries. Quay `GLOBAL_READONLY_SUPER_USERS` reads **robot-account tokens for repositories they don't belong to** → impersonate any robot (CVE-2026-18255) — "read-only" super roles layer reads over resources and forget credential-view sub-resources; sweep token/secret sub-resources under any read-only admin concept. + folds: **sequoia-openpgp absent Key Flags subpacket → inferred capabilities → back-signature check bypass → arbitrary subkey bind → signature forge** (CVE-2026-42784, diff absent-field defaults across OpenPGP/X.509/SAML consumers) onto the Bouncy Castle crypto-boundary page; RHOAI training-operator **aggregated rules on stock `edit`/`admin` ClusterRoles + PodTemplateSpec passthrough** → SA impersonation/host-fs/RCE from vanilla namespace roles (CVE-2026-18982/18951), DSPO **weak-PRNG-derived MariaDB root + MinIO keys offline-derivable from any Route/Service reach** (CVE-2026-18611), DSPO over-scoped ClusterRole pod-exec/role-mgmt as the post-foothold cluster-admin ladder (CVE-2026-18608) onto the Aug 10 AI control-plane page. Tracked: libaom SVC crash-oracle RCE quartet, xdgmime user-writable magic-file OOB write, AAP group-writable /etc/passwd container-root, ml-metadata H2 DoS](2026-09-21-automation-platform-credential-test-exfil-and-fqdn-endpoint-metadata-ssrf-ghsa.md)

- [**Workflow-platform internal deputies + presigned-URL operation confusion** (Sept 21 12:32Z wave, new page): Temporal completion callbacks decide **internal vs external from a caller-supplied `source` header** — non-empty → History re-targets at the **internal frontend client** (authorizes every request as system admin, no auth), rewriting only scheme+host so caller path/query/body ride along → attacker-chosen admin-API POSTs in namespaces the caller can't touch (terminate foreign Workflows, register/delete namespaces); precondition probes = `services.internal-frontend.rpc.httpPort` non-zero + per-namespace `component.callbacks.allowedAddresses` non-empty; 1.30+ any non-empty header, 1.25–1.29.7 needs exact cluster-ID match (CVE-2026-87858). Temporal Worker Controller **`subprocess` compute provider takes program+argv from the caller's request**, and `workercontroller.compute_providers.enabled` **skips the allowlist check when unset (the default)** → namespace-write role → immediate RCE as the server account holding all-namespace persistence creds + cluster TLS; **execution-on-save** (eager invoke after validation) (CVE-2026-89139, 1.31.0–1.31.2). NooBaa SigV4 **drops unsigned `x-amz-` headers instead of rejecting** → presigned PUT + unsigned `x-amz-copy-source` = server-side CopyObject of anything the signer can read — signature still valid, operation changed underneath it (CVE-2026-94368). Axes: **caller-supplied headers that select internal trust context; default-unset allowlists enforce nothing; config-in-request is code-in-request; presigned URL scope ≠ granted operation**. + CRI-O checkpoint-restore security-state retention fold onto the June 19 page (credentials/capabilities/no_new_privs/seccomp survive into the restored process despite the destination Pod spec, CVE-2026-92574, 8.8)](2026-09-21-temporal-internal-deputy-subprocess-provider-and-noobaa-presigned-copyobject-ghsa.md)

- [**SAML assertion attack-surface testing** (new methodology page, from the Sept 21 Trail of Bits "SAML: A fractal of bad design" post inverted into an operator target map): six independent parser layers between assertion bytes and the authorization decision — XML/DTD (XXE/SSRF via canary DTDs), canonicalization (comment/whitespace/attribute-reorder family), enveloped signature (XSW placement + Reference/Digest mutations), conditions (`OneTimeUse`/`AudienceRestriction`/`SubjectConfirmationData` stripping matrix), binding parity (POST vs Redirect vs artifact verdict tables), and identity linking (asserted-email vs configured-criterion resolution). Mutation-battery order, metadata-fingerprint-first recon, the Go round-trip asymmetry leg, and the shape-normalization defense inverted as a bypass target (mutate only inside the grammar the normalizer tolerates). Report language: **which copy of the document the signature check vs the identity check read**](../methodology/saml-assertion-attack-surface-testing.md)

- [September 21 three-page fold wave: **Keycloak 4-GHSA follow-up** (fold onto the June 11 Keycloak page) — Admin REST resolves clients by UUID from a per-request cache **without realm-path ownership check** → low-scope realm admin reads/patches **master-realm** client config (CVE-2026-94215); policy-evaluation **dry-run endpoint enforces no user-read authz** (CVE-2026-94213); UMA merges permission tickets of **same-named resources across owners** (CVE-2026-94217); mid-login **session-restart link wipes required-action markers** → 2FA-setup policy bypassed (CVE-2026-94218). Axes: cache key missing the scope dimension, test/preview route families skip prod authz, name-keyed aggregation crosses owners, flow-restart affordances clear security state. **WP 5-GHSA fold** onto the Sept 19 alternate-surface page — PayPal gateway IPN verifies genuine transaction but **not environment/merchant-account** → self-mark order paid with your own sandbox ($0 legitimacy-tuple test CVE-2026-92400); GiveWP **one-pass shortcode strip defeated by nesting**; To Do List Member unauth no-nonce import with caller-chosen fetch target; **ScadaBR import gated with ensureAdmin() but export left open** = mirror-leg sweep exemplar (CVE-2026-94148); RestroPress client-price trust; upload token handed to any visitor + zero validation = unauth PHP RCE (CVE-2026-82187). **CRI-O checkpoint-restore metadata → host filesystem** folded onto the June 19 containerd/checkpoint page (CVE-2026-15801, 8.0): checkpoint bundle = untrusted input with host-side parsing, feature-flag enablement + who-can-trigger-restore are the precondition probes](2026-06-11-kolibri-hapi-keycloak-flowise-arc-boundary-batch-ghsa.md#september-21-keycloak-follow-up-cache-scope-debug-endpoint-authz-name-keyed-merge-and-session-restart-marker-wipe-4-ghsas)

- [Argo Workflows Strict/Secure allow-list depth-one bypass (fold onto the May 4 Argo page): the CVE-2026-31892 fix validates user spec overrides by **reflection over top-level `WorkflowSpec` fields only** — `ArtifactGC` is allow-listed wholesale and its `PodSpecPatch` sub-field flows unmodified into the same `ApplyPodSpecPatch` sink the fix closed → under `templateReferencing: Strict/Secure`, any referenced template with an output artifact + submitter-set `artifactGC.strategy` gets attacker image/command, `privileged: true`, `hostPath: /`, `hostNetwork: true` on the artifact-GC pod (auto-mounted SA token). Axes: **depth-one allow-lists enforce nothing — every allow-listed field is a subschema**; enumerate every other call site of the patched sink helper; the fix's regression-test file (top-level field only) is the attack map; advisory ships clusterless Go unit-test PoC harness; fixed 4.0.6/3.7.15 (CVE-2026-54526/GHSA-48p8-g2fx-3wwm)](2026-05-04-argo-workflows-auth-resource-boundary-batch-ghsa.md#september-21-follow-up-the-strictsecure-allow-list-fix-was-depth-one-only-artifactgcpodspecpatch-reaches-the-same-pod-patch-sink)

- [September 21 ML-loader fold: **"fixed" narratives lie about pickle loaders** — stable-baselines3 `PPO.load`/`load_replay_buffer`/`VecNormalize.load` still unpickle by default because the `weights_only=True` hardening was **reverted on master** for PyTorch compat, and the earlier fix gated only the *separate* `huggingface_sb3` hub loader (CVE-2026-94093); gensim `SaveLoad.load` unsafe at develop HEAD with the issue closed same-day unfixed (CVE-2026-94091); DGL `load_info` unguarded (CVE-2026-94092). Axes: fingerprint the **installed loader call** (grep `weights_only`/`RestrictedUnpickler`), not the changelog; a fix in package A proves nothing about sibling APIs in package B; treat model/replay/normalizer/graph artifacts as code-execution channels on stagnant ML repos — folded onto the June 30 model-parser/deserialization page with a bounded canary-loader matrix](2026-06-30-model-parser-deserialization-identity-boundaries-ghsa.md#september-21-follow-up-ml-library-loaders-still-pickle-load-by-default-and-fixed-narratives-lie)

- [September 21 MCP/upload fold (2 pages): **ACE-MCP `get_file_snippet` containment guard anchored to a caller-supplied root** — `isPathInsideProjectRoot` only stops `filePath` escaping the *attacker-chosen* `projectRootPath`; set root=`/etc` + path=`hosts` and the guard passes → when both legs of a containment check are client input, sweep sibling roots by simply *choosing the root* (CVE-2026-94046, folded onto the Sept 15 MCP page with same-wave MCP singles: `create_file` unsandboxed write, MCPHub `importTemplate` privilege drift, pdf-tools-mcp `pdf_file_path` SSRF = a PDF-tool path argument is a fetch/file sink); **newbee-mall `ImageIO.read()` image guard** is format-agnostic → polyglot PNG+HTML tail passes validation, attacker suffix + `/upload/**` static mapping = stored XSS — evidence the validation/storage/delivery legs separately (CVE-2026-94045, folded onto the Sept 20 media-pipeline page)](2026-09-15-mcp-dns-rebinding-alternate-auth-path-and-sandbox-boundaries-ghsa.md#september-21-follow-up-containment-guard-anchored-to-a-caller-supplied-root-enforces-nothing)

- [OrdaSoft Joomla Gallery four-GHSA wave (fold onto the July Joomla boundary page): `$input->getVar()` is **not a real Joomla filter method** — the call falls through to a permissive tag-strip default, quotes/SQL survive into a concatenated LIKE clause → **unauthenticated UNION SQLi from the public search module** (CVE-2026-88854) — nonexistent filter-method names fail *open*; `update_osgallery` JSON body **calls the value of a `method` field as a live PHP function** with `package` as its argument, no allowlist (`system`/`exec` reachable, CVE-2026-88856); `saveWatermark()` copies uploads with the **client filename verbatim** into a web dir (`.php` + image Content-Type disguise, CVE-2026-88857); `saveGallery()` ARRAY/STRING filter types sanitize nothing → component-scoped `core.manage` crosses into full-DB read/write incl. `#__users` hashes (CVE-2026-88855). Axes: caller-supplied *names* resolving to functions/verifiers = remote method dispatch; component-scoped grant ≠ data-scoped ceiling; filename-preservation sweep on every save/export/image-tool leg; login-to-RCE ladder via one component grant](2026-07-08-joomla-page-builder-coldfusion-kev-boundaries.md#september-20-follow-up-ordasoft-gallery-filter-fail-open-method-field-dispatch-and-raw-filename-watermark-4-ghsas)

- [**REDCap survey-passthrough route confusion → unauthenticated RCE** (Sept 20, CVE-2026-90817/GHSA-hg88-qg2r-c8mx, 13.3.0+): a valid **public survey hash** gates a passthrough router that can be manipulated to reach an **unintended controller route**, and the Data Import handler accepts a crafted **file-path/stream parameter** → RCE with no login. Durable axes: **a public token authenticates a context, not the internal route family behind it** — sweep route/action selectors from any token-gated passthrough; import handlers' path/stream params are file sinks; hash *predictability/enumerability* is a separate finding class that collapses the precondition. Dispatch-evidence-only validation scaffold + generalization checklist for survey/form/share-link platforms](2026-09-20-redcap-survey-passthrough-route-confusion-unauth-rce-ghsa.md)

- [Media-pipeline edge surfaces wave (Sept 20 12:30Z): **getID3 filename shell-out** — probe handlers build command strings with the **filename unescaped** → a media file named with metacharacters executes commands in any embedding PHP app (8.8, CVE-2026-94106): fuzz the *name*, not just the content; **getID3 `XML2array` XXE** — XML metadata **inside a media container** is parsed with entity loading enabled → file read/SSRF via the metadata library, no XML endpoint needed (CVE-2026-94108); **openEQUELLA FreeMarker SSTI** — unsandboxed `TemplateClassResolver` on collection summaries/portlets/MIME templates = content-author → `Runtime.exec` (8.8, CVE-2026-94109); **NivoCart chunked-upload validation asymmetry** — `multi()` skips extension checks when `chunks ≥ 2`, view-only user → PHP upload → RCE (8.8, CVE-2026-94104) — run the parameter-selected validation branch, not just the default. + folds: BrowserSkill **shape-predicate extension-origin allowlist** forgeable by key brute-force onto the Sept 15 MCP page (CVE-2026-94111), NivoCart `md5(mt_rand())` tokens + ezBookkeeping **replayable TOTP** + Dancer2 **silent CSPRNG fallback** + Frappe whitelisted-RPC permission drift onto the Sept 18 token-entropy page (4 GHSAs), SmartLife hardcoded decrypt key marked processed (CVE-2026-86555)](2026-09-20-media-pipeline-edge-surfaces-filename-shellout-metadata-xxe-author-template-rce-ghsa.md)

- [WordPress validation-loop corruption + un-fuzzed deserialization transports (Sept 20 09:30Z): NextGEN Photo Gallery archive validator **reuses the loop counter as the validation result so the extension check always passes** → arbitrary web-dir file write (CVE-2026-81650) — test validators with **mixed allowed+forbidden batches**, single-item tests can't tell a dead loop from a working one; Forminator **deserializes XML-RPC request values with no class restriction** (CVE-2026-87067) → run the PHP-object-injection battery on `xmlrpc.php`, not just POST params; Forminator quiz import accepts a nested admin-granting registration form the ordinary editor **and** ordinary import both refuse (CVE-2026-87068) → nesting-parity verdict tables; Unlimited Elements AJAX deserializes stored data, Subscriber POI, and the 2.0.18–2.0.19 "fix" only raised the role (partial-fix differential — fingerprint minor versions); SAML SSO plugin **ignores the configured linking criterion, always binds by login name** → IdP-assertion ATO up to admin (CVE-2026-82842) — field-binding decision tables with a lab IdP; TikTok callback redeems any visitor's supplied sign-in code with the **site's own** platform credentials (CVE-2026-92965). 11 promoted GHSAs + SmartLife verify.serv twin marked processed on the Sept 20 app-auth page](2026-09-20-wordpress-validation-loop-corruption-deserialization-transport-and-assertion-override-ghsa.md)

- [App-auth forgery + credential-vault platform wave (Sept 20): **SmartLife/ZTE backends** — client-runtime app-authentication params are forgeable → `/account/verify.serv` email→account-ID oracle + password reset trusting the **asserted** account ID = full ATO (8.5, CVE-2026-86553), plus signup accepting **any unowned email** with no ownership check (CVE-2026-86552). **Devolutions Server (4 GHSAs)** — vault SSRF via low-priv user-submitted connection definition in VMware datacenter discovery, leaking other users' credentials + metadata reach (CVE-2026-90971); entry-listing endpoint honors **password-disclosure params without the view-password permission** (CVE-2026-90969); LDAPS + shared sync HTTP client accept spoofed certs → intercept privileged DC creds (CVE-2026-13327/84850). **HPE SD-WAN Orchestrator** — **read-only** user + crafted cache-sync request discloses third-party integration API tokens (9.9, CVE-2026-76672). Axes: app-auth params are service-level capabilities, permission checks belong on endpoints not UIs, read-only roles reaching token-bearing integrations](2026-09-20-app-auth-forgery-chains-and-credential-vault-platform-boundaries-ghsa.md)

- [Grammar-desync pair (Sept 20): **Exim SMTP smuggling via data sent after a DATA-phase rejection** — the delivered message matches neither transaction sent, test with a three-way sent-pre/sent-retry/delivered diff on an owned MTA pair (CVE-2026-94057); **Expat UTF-16 lone high surrogates consume the following code unit, hiding `<`/`>`/`&` markup from one decoder while a second assembles it** → re-run XXE batteries with invalid surrogate sequences on any UTF-16 XML entry point (CVE-2026-93990). Family rule: buffer/decoder state that survives the boundary that should have killed it. + 3 folds: Argo NotEquals field-selector authz drop, Mistral Vibe `post-checkout` hooks before trust, Exim PROXY-parser memory safety](2026-09-20-exim-smtp-rejection-smuggling-and-expat-surrogate-markup-hiding-ghsa.md)

- [Keycloak delegated-administration and lifecycle-state follow-up (fold, 3 GHSAs): Admin REST credential-deletion endpoint skips the fine-grained **reset-password** permission → restricted delegate deletes user credentials (CVE-2026-94001); OIDC refresh restores audiences from **stored** client IDs without rechecking **enabled** state → refresh-token holder keeps minting access tokens for a disabled client, decisive against offline-JWT resource servers (CVE-2026-93999); group-membership write skips the target group's **admin-privilege class** → minimal delegated admin self-adds to a high-privilege group (CVE-2026-94000). Durable axes: verb-family permission diffing, disabled/revoked-state recheck on every mint/refresh/exchange path, and privilege-class gates on membership writes](2026-06-11-kolibri-hapi-keycloak-flowise-arc-boundary-batch-ghsa.md#september-19-keycloak-delegated-administration-and-lifecycle-state-follow-up-3-ghsas)

- [JS sandbox-validator + async-render escape wave (12:32Z): OpenPanel js-runtime webhook-template validator misses **computed member access to constructor chains** → project-write privilege becomes worker-process RCE via `["constructor"]["constructor"]("code")()` (9.9, CVE-2026-93985/GHSA-rqpr-2wvr-29pf) — the JS twin of the Sept 17 Python guard lesson: after any template/expression-sandbox fix, enumerate **every language alias reaching the same power** (computed access, `Function`/`globalThis` chains, per-normalization spellings); ClickHouse **filter-key** SQL injection = project-isolation bypass (values parameterized, keys not — key-position sweep on any filter-name API); tracking-API client secret **hash never verified** (presence-only auth); hono/jsx `< 4.13.7` plain strings **unescaped on async composition paths** — `Suspense` child/fallback, `ErrorBoundary` with async sibling, single `Context.Provider` child, `renderToString`/`renderToReadableStream` root — escaping is a property of the render path, not the framework (4.7, GHSA-6623-4q32-82v8)](2026-09-19-js-sandbox-validator-constructor-chain-and-async-render-escape-ghsa.md)

- [WordPress alternate-surface authorization drift wave (09:32Z, 17 durable GHSAs): **unauthenticated REST routes with no permission_callback** — Botiga Pro arbitrary-options write + site-wide stored scripts + arbitrary post trashing; Master Blocks unauth settings write → **admin-session stored XSS in wp-admin**; Mailchimp for WooCommerce admin endpoints reachable anonymously; **MgoSync hands out read/write WooCommerce API keys unauthenticated**; AI booking-assistant conversation IDs unowned → read **and inject into** another visitor's live AI conversation; UsersWP social login resolves accounts by **asserted email without provider ownership confirmation** → ATO up to admin; WP Import Export Lite delegated-import permission → **create admins / overwrite credentials** (delegated cap ≠ implied core cap); Secure Custom Fields unauth submit against any registered form; Custom Field Template `current_user_can` **int-casts the ID the authz check sees while the string reaches SQL** (cast-differential); The Welcomizer nonce-only handler + `eval()` + nonce minted by a public JS asset → Subscriber RCE; CF7-addon arbitrary-extension upload at predictable path (`.phar` → RCE on Debian/Ubuntu Apache default handler mapping — handler parse proof required for the RCE claim); Ultimate Member sanitize-**then**-entity-decode stored XSS; JetFormBuilder blocklist omits a file-deletion function, executed at unauthenticated form submit (deferred-execution callback)](2026-09-19-wordpress-alternate-surface-authz-drift-rest-ajax-import-wave-ghsa.md)

- [OpenShift console September 19 follow-up (fold onto the Aug 10 AI/control-plane page): unauthenticated `/api/devfile/` + `/api/devfile/samples/` SSRF — crafted devfile payloads make the console pod fetch cluster-internal services and **reflect partial responses** (CVSS 9.3 scope-changed, CVE-2026-75885). Axis: enumerate fetch-bearing console route families by feature and probe each unauthenticated — a patched webhook-helper SSRF says nothing about the devfile sibling family; paired missing-content-length memory DoS tracked not published](2026-08-10-ai-feature-control-plane-tenant-boundaries-ghsa.md#september-19-follow-up-unauthenticated-devfile-endpoint-family-keeps-console-pod-fetch-authority)

- [WordPress protective-transform-inverted wave: PDFCrowd Save-as-PDF — server **AES-encrypts an attacker-chosen `pdf_created_callback`** shortcode attribute and the anonymous `wp_ajax_nopriv_save_as_pdf_pdfcrowd` endpoint decrypts and **invokes it as a PHP callable** (8.8, CVE-2026-92807 — the server hands any Contributor a cryptographically valid blob; encryption ≠ sanitization); Gravity Forms 9.8 (CVE-2026-84434) — **hidden File Upload fields skip extension validation** while the persist leg still passes the rejected upload state to `upload_file()` → unauthenticated arbitrary upload; WP Recipe Maker 9.1 (CVE-2026-89274) + Forminator 9.1 (CVE-2026-92229) — `do_shortcode()` executes untrusted comment/action text **before** `strip_shortcodes()` runs on the output, JSON-LD `reviewBody` = built-in disclosure channel; Better Messages (CVE-2026-89093) — privileged internal "AI bot" principal **prefix-matched from client-controlled `X-Real-IP`** short-circuits `user_can_join()`/`user_can_read()`. Axes: **encrypt-then-dispatch field semantics, validate-leg ≠ persist-leg, sanitize-after-execute ordering, header-derived internal principals**; + folds: Divi Essential conditional-nonce (`nonce` checked only when present) full-DB table/row reader onto the July 28 WP page (CVE-2026-15760), WPPA+ whole-command `escapeshellcmd()` argument injection onto the Aug 5 command-wrapper page (CVE-2026-87909) (5 GHSAs + 2 folds)](2026-09-19-wordpress-shortcode-execution-hidden-field-upload-and-encrypted-blob-dispatch-ghsa.md)

- [ORM query-grammar injection + authorization-target fallback: Mongoid cluster — caller-supplied **field names reflected into internal method invocation** on embedded-doc query + persistence paths (CVSS 9.8/9.1: disclosure + record removal), **string query criteria forwarded to MongoDB as server-side JavaScript**, unrestricted operators from caller-supplied filters, nested-attributes **IDOR-with-persistence-bridge**, and CSFLE schema generation **silently omitting embedded-model fields → "encrypted" columns stored cleartext** (6 GHSAs); OpenStack Blazar policy wrapper resolves the authz target under keyword `lease_id` while the router delivers `id` → lookup returns None → **authorization falls back to the caller's own project** → any user mutates/deletes any tenant's leases, chained with unscoped `GET /v2/leases` that hands out the IDs (CVE-2026-93854/93852); Cotonti `md5(microtime())` recovery tokens ≈10⁶/sec precomputable from the `Date` header → admin ATO (CVE-2026-93868) + `unserialize()` without `allowed_classes` on a base64 comment param (CVE-2026-93872); Azkaban `fetchSchedule` skips the permission check its sibling actions have → cross-project flow parameters + cron + email lists. Axes: **key-position injection, authz-target-name drift, read-bug-feeds-write-bug chaining, clock-derived token entropy scripts, per-action authorization matrices**; + folds: http-cache-semantics `Vary` byte-comparison + `max-stale`-over-zeroed-entries onto the shared-cache page (CVE-2026-93750/93748), LightLLM unauthenticated `/pd_register` node hijack → prompt disclosure (CVE-2026-93839, 9.8) onto the May 21 AI-runtime page (11 promoted GHSAs + 2 folds)](2026-09-18-mongoid-orm-query-grammar-blazar-authz-fallback-and-cotonti-predictable-tokens-ghsa.md)

- [React Router file-session fold (CVSS 9.1, CVE-2025-61686/GHSA-9583-h5hc-x8cw): `createFileSessionStorage()` with an **unsigned cookie** makes cookie contents attacker-controlled input used as a file-path component → traversal reads/writes outside the session dir, bounded by server-process permissions; reads are format-gated (no direct oracle — trace session-key reflection before claiming disclosure), writes are the concrete integrity axis. Reusable axis: **grep framework cookie factories for missing `secret` — an unsigned cookie is fully attacker-controlled input at every downstream sink**; folded onto the June 3 React Router boundary page](2026-06-03-react-router-redirect-rsc-and-manifest-boundary-batch-ghsa.md#september-18-follow-up-unsigned-session-cookie-as-a-file-path-sink)

- [Scope parameters that are accepted but never enforced: kcp front-proxy **appends instead of replacing** `X-Remote-*` identity headers → any tenant asserts `system:masters` on the shard (CVSS 9.9, CVE-2026-61682); Convoy `FindSourceByID` takes the project ID and never uses it — SQL has no `project_id` predicate → any-project caller reads any project's Sources with **plaintext broker credentials** (CVE-2026-81505, no patch); Perses `?project=` query param re-points the lookup past the path-authorized project + list-query traversal skips the body-only `Flatten` validation + datasource proxy decrypts Secrets you have no scope on (3 CVEs); Moquette Will-message path skips `canWrite()` → anonymous ACL-restricted topic injection via TCP RST (CVE-2026-85058); Obot DCR+no-consent+audience-ignored group-carrying token = one-click API-wide token theft (8.8) + auto-reconcile non-blind metadata SSRF + default-allow authorizer prefix drift; AnyCable `body_md5` signed but never recomputed = infinite body-substitution replay; zot maps DELETE→"push" and bearer bypasses the fine-grained authorizer. Axes: **list-vs-get authz diff, query-shadows-path probe, read-vs-write validation asymmetry, method-to-action collapse, broker side-channel publish** (11 promoted GHSAs + 2 folds)](2026-09-18-ignored-scope-parameters-kcp-identity-header-smuggling-convoy-perses-obot-ghsa.md)

- [Semantic MediaWiki server-side boundary wave (11 GHSAs): `api.php?action=smwtask` has **no authorization check** — anonymous users run admin-only maintenance tasks using the fixed public CSRF token `+\` (a token check is not an authorization check); `Special:Ask` cursor pagination is **unsigned base64url JSON** → forged `sort_prop` reflects into raw `Html::errorBox()`; `format=debug` echoes query/SQL/EXPLAIN unescaped; `Special:FacetedSearch` `cstate` gate is `csum==crc32(q)` — compute-and-bypass; `Special:URIResolver` interwiki-prefix open redirect. Reusable axis: **every alternate programmatic surface (API module, debug format, raw output mode, pagination token, checksum gate) is a separate trust boundary** (8 promoted GHSAs)](2026-09-18-semantic-mediawiki-unauth-api-tasks-unsigned-cursor-and-debug-sinks-ghsa.md)

- [LMDeploy DistServe trio folded onto the May 21 AI-runtime page: `POST /distserve/p2p_connect` lets an unauthenticated caller **dial the engine's own ZMQ client socket out to an attacker listener** that pickle-deserializes it (CVSS 9.8, CVE-2025-66455); `eval(f'torch.{quant_dtype}')` on model config — sibling whitelist covers `torch_dtype` only, comma-tuple payload survives the `torch.` prefix (CVE-2026-33625); `\@` urlparse-vs-requests host differential defeats the `is_global` SSRF guard (GHSA-39wr-7q6h-cf68). + Capsule admission-bypass trio folded onto the May 28 Capsule page: case-insensitive-sort + byte-order binary-search makes `ExactMatch` miss keys literally in the forbidden denylist (vary key case on any exact-match denylist, CVE-2026-61672); OnUpdate webhook validates the **OLD** object (pair valid-old + invalid-new on any update path); sibling-field compile-miss in validation loop](2026-05-21-svelte-markdown-sagemaker-lmdeploy-boundary-batch-ghsa.md)

- [XWiki critical fold (CVSS 9.9, CVE-2025-53837/GHSA-26vp-8gxg-v4pg): extension-point renderings are re-embedded inside the enclosing `{{html}}` macro **without escaping** → any user who can edit their own profile emits `{{/html}}` (tilde-escaped so it survives the containment pass) + live `{{groovy}}` → **programming-rights RCE, full wiki read/write**, back to 3.3 M1, fixed 14.10.2/15.0 RC1; reusable axes: macro-close injection wherever renderer output re-enters a raw-content macro, extension-point registry as sink-enumeration surface, escaped-spelling sweep on any wiki-syntax validator — folded onto the June 22 XWiki/wiki-macro page](2026-06-22-container-openam-xwiki-comfyui-boundaries-ghsa.md)

- [Media client-side render sinks as stored-XSS carriers: Opencast Paella player appends WebVTT/DFXP cue text with `innerHTML += cue` → a non-admin author's subtitle upload executes in every viewer's session (anonymous + staff) on **default config**, caption file served raw to anonymous clients via `/search/episode.json` (CVE-2026-77615/GHSA-m6c8-jcw2-5r25); auxiliary-asset formats (captions, transcripts, chapters, playlists) are an input channel form fuzzers never touch. + Semantic MediaWiki stored XSS (CVSS 8.6): `ext.smw.js` JSON-parses the **unregistered** `data-subtab` wikitext attribute and concatenates it into `innerHTML` → any `edit`-right user gets stored execution against all readers (CVE-2025-61682/GHSA-hg8h-557g-q8pp); grep bundles for the `dataset`→`JSON.parse`→`innerHTML` chain and sweep unregistered `data-*` names past reserved-name-only sanitizers. + Caddy replacer-layer trio folded onto the July 24 Caddy page: `rewrite` trailing-`?` placeholder **double-expansion** leaks `{env.*}`/`{file.*}` into URL/logs/upstream — the exact `vars_regexp` gadget (CVE-2026-30852) unextended to sibling directives (CVE-2026-77281); case-sensitive `hide` matcher vs case-insensitive APFS/NTFS → `.GIT`/`.ENV` served despite hide rules; `{http.request.body}` placeholder read bypasses `max_size` (3 issues, 2 GHSAs)](2026-09-18-opencast-caption-track-stored-xss-carrier-ghsa.md)

- [New methodology page — **Agent-built audit tooling for custom VMs and DSLs** (Trail of Bits Miden zkVM write-up, Sept 18): agent-built LSP → subset decompiler/IR → abstract-interpretation passes → Lean formal model as review prep on no-tooling bytecode targets; the headline reusable vuln class is **untrusted witness operands coupled only by a reconstruction equation** — `mod_12289` range-checked the prover-supplied quotient but never the remainder, so a malicious prover trades slack between the pair to return a wrong remainder → forged Falcon signatures → account drain; sweep every advice-pair / (length, checksum) / (offset, size) crossing a verify boundary; failed-proof-missing-precondition as bug oracle (`rotr` shift-mod-32 edge case)](../methodology/agent-built-audit-tooling-custom-vm.md)

- [Sept 18 09:31Z WP plugin wave folded onto the July 28 page: **Booking Calendar Editor→Admin** via name-keyed option-policy registry that returns an empty policy (all checks bypassed) for unregistered option names → writes `default_role=administrator` with a trivially-fetched request-supplied-action nonce (CVE-2026-92619); **Hide My WP Ghost** security plugin disables its own firewall/URL-hiding when a request contains an attacker-suppliable WooCommerce parameter or a missing/wrong loopback check value — protection-layer inversion of the parameter-ABSENT axis (CVE-2026-86796/86800); Auto Upload Images `wp_remote_get` with reject-own-hostname-only validator = private/link-local SSRF (CVE-2026-12106); Clean Login CAPTCHA skipped when session state empty + registration-created-without-checking-registration-enabled (CVE-2026-90976/90977); + Airflow asset queued-events DELETE gated on `READ` instead of `EDIT` folded onto the Sept 17 page — verb-permission mismatch sweep (CVE-2026-75157); Filter Gallery nonce-omission sharpening, Mapster meta-write, Synology DSM + HCL BigFix clusters tracked (77-advisory wave)](2026-07-28-wordpress-payment-device-boundaries-ghsa.md#september-18-0931z-follow-up-empty-policy-fallthrough-to-admin-security-plugin-fail-open-and-validator-shape-ssrf)

- [WordPress core 7.1.1 security release: **unauthenticated stored XSS in `wpautop()`** reproducing on a default install (moderation off by default + previously-approved requirement bypassable; CVE-2026-93485) + HTML-API `set_modifiable_text()` comment-breakout, XML-RPC publishing `customize_changeset` posts bypassing `edit_css` (transport-parity drift), crafted-URL theme install/preview, contributor+ arbitrary post overwrite + draft/pending slug disclosure, attachment metadata leaking private parent titles, any-user comment reparenting (11 core fixes); + Sept 18 06:31Z plugin wave folded onto the July 28 page (Ai1wm export-disclosed installation secret → import → admin, Easy Appointments timestamp-derived token, CF7 PDF-renderer SSRF with artifact response channel, VikBooking chat-attachment admin-viewer sink, Filter Gallery nonce-present-only check)](2026-09-18-wordpress-core-7-1-1-parser-xmlrpc-and-object-scope-boundaries-ghsa.md)

- [Nextcloud app-layer authority drift: Circles signature verification fetches the attacker-supplied `keyId` URL **before** trust is established and explicitly allows local/private addresses, bypassing core SSRF protections from two unauthenticated POST endpoints (blind reachability oracle; CVE-2026-77164); WebDAV lock/unlock resolves files from the absolute request URI with no session-owner check → cross-user locks + lock-token disclosure (CVE-2026-82980); Deck board-config API skips the ownership check every sibling route has (CVE-2026-77170); Photos smart albums evaluate the **viewer's** source-folder config against the owner's tree → shared-album metadata oracle (CVE-2026-82985); Approval etag freshness check runs **only if the client sends the etag** (CVE-2026-82982); Workspace-delegated admins bypass folder authz over REST (CVE-2026-77169); + yt-dlp-web-ui `params`→argv command injection folded onto the Aug 5 command-wrapper audit (CVE-2026-93371) (7 GHSAs)](2026-09-18-nextcloud-app-layer-authority-drift-circles-keyid-ssrf-webdav-locks-and-deck-config-ghsa.md)

- [Extraction-before-verification and client-supplied proofs: Grafana OSS/Enterprise extracts plugin archives **before signature verification** — chained relative symlink entries escape the plugin dir, write arbitrary files + an executable backend binary that runs as the Grafana server process (RCE); a valid signature does not prevent the write, and the legit-appearing-plugin / `GF_INSTALL_PLUGINS` / preinstall paths are all exposed (CVE-2026-15815); SOGo builds password-reset links from the client-supplied `Origin` header → reset token mailed to the victim inside an attacker-authority link → ATO (CVE-2026-93453, folded onto the May 19 Host-header page); Amelia `/users/customers/<id>` no ownership check → provider-role password reset of any customer = WordPress core ATO up to Editor (CVE-2026-14311) + Motors admin-ajax leaks draft/pending/private/future listings unauthenticated (CVE-2026-16750) — both folded onto the July 28 WordPress page (5 GHSAs)](2026-09-18-grafana-extract-before-verify-sogo-origin-reset-and-client-supplied-proofs-ghsa.md)

- [Grav 20:23–20:59Z wave folds: four Twig-sandbox read primitives (`system`/`site`/`theme` raw arrays policy-invisible, `config_denied_paths` missing `system`, unfiltered `offsetGet` leaks password hashes/2FA seeds, `media_directory('/etc')` + `Medium.filepath` arbitrary file read); dynamic-data ladder closes to login-to-RCE (dual-form `[class,method]` callable bypasses `isSafeDynamicCall`, `Installer::unZip` to docroot, config-file-as-own-blueprint; bare-function denylist omits `error_log` = arbitrary file write); `detectXss()` engine fail-opens on one invalid UTF-8 byte (`preg_match /u` returns false ≡ clean) + unpaired-quote tokenizer gap introduced *by* the prior quote-aware fix; sibling-group-blueprint super-escalation + zero-consumer `check_authorize` illusion control; pre-boot `plugin-asset-map.php` sibling-prefix traversal; djust template layer auto-escape parity defects + name-keyed safety grants; CoreDNS DoH/DoQ/gRPC relay unsigned UPDATEs (transport × policy table); @platejs/core live-document HTML parse fires before sanitization (18 GHSAs)](2026-08-25-grav-sandbox-escape-and-privilege-host-origin-boundaries-ghsa.md)

- [User input as CLI argv / platform-structural sinks: MISP events/contact form forwards `person`/`message` into CakePHP console **argv**, where `ShellDispatcher::_parsePaths()` honors `-app`/`--app`/`-root`/`-webroot` from any argv element → `person=-app` + `message=phar://<uploaded archive>` → bootstrap includes attacker `Config/core.php` → deterministic unauthenticated RCE; SAP `@sap/cds-mtxs` (CVE-2026-76969, critical) unauthenticated tenant-credential disclosure on extensibility-enabled CAP multitenant apps → tenant data replace/delete; Verizon Cloud for Android exported `OneTouchUploadActivity`/`PrintShopCloudActivity` unsanitized `_display_name` path traversal → arbitrary file write from any co-resident app; + MISP Overmind statistics legend innerHTML stored XSS folded into Sept 7 page (4 GHSAs)](2026-09-17-misp-cakephp-argv-phar-rce-sap-cds-mtxs-tenant-credentials-and-android-exported-activity-file-write-ghsa.md)
...
- [Yamcs StreamSQL Janino RCE, function-AC fail-open wave, and Phalcon router ReDoS (11 GHSAs)](2026-08-28-yamcs-streamsql-janino-rce-and-funcac-fail-open-wave-ghsa.md)
