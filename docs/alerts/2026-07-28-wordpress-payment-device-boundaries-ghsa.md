...
## September 17 06:30Z follow-up: issuance registries, failure-path side effects, request-derived authority, and the `is_admin()` myth

A September 17 06:30Z wave of WPScan-derived advisories adds four reusable authentication-boundary patterns and a large tail of already-covered-axis singles.

### Token checks that search a store instead of an issuance registry

Two nearly identical records expose one generalizable break. **Login with QR through 1.0.0** [GHSA-37px-v36p-7jmx / CVE-2026-86710](https://github.com/advisories/GHSA-37px-v36p-7jmx) and **Private Feed Key through 0.1** [GHSA-3m5h-9m27-j5hm / CVE-2026-86707](https://github.com/advisories/GHSA-3m5h-9m27-j5hm) both authenticate a login by matching the submitted code/key against **any stored user-metadata value** rather than against a value the plugin itself issued — so an unauthenticated attacker who knows (or can guess/leak) *any* user-meta string logs in as that user, including administrators.

The audit rule: an authentication check must prove **issuance provenance**, not mere existence. A lookup of the form "find user WHERE meta_key LIKE any-meta WHERE meta_value = submitted" is broken even when the comparison is constant-time and the code space is large, because the *matching set* includes unrelated metadata the user never authorized for this flow. On any WordPress login-adjacent handler (QR login, magic link, feed key, device pairing, invite redemption):

1. Ask where the expected value is *stored* — a dedicated plugin option/transient with expiry, or a shared `wp_usermeta` table? The second is a red flag.
2. Enumerate what else can land in that same store (profile fields, other plugins' meta, importable fields) — every writer into the shared store is a potential issuer you do not control.
3. Prove on a disposable site only: set one synthetic user-meta value via `update_user_meta()` in a lab shell, submit it as the login code, and record whether a session is minted. The bounded positive is **anonymous request with an arbitrary meta-sourced string -> authenticated session for the owning user**. Never test with real user metadata or retain a minted session.

**Pressengine through 1.0** [GHSA-r5f2-c2h7-4jxh / CVE-2026-86709](https://github.com/advisories/GHSA-r5f2-c2h7-4jxh) is the complementary failure-path lesson: its login handler **issues a session even when authentication fails**, allowing login as any user. Generalization: every failed-authentication branch must be checked for state side effects (session issuance, cookie set, token store write, attempt-counter reset). Harness: submit a deliberately wrong credential through the plugin's alternate login route on a lab install and verify no session cookie or `wp_users`-linked state appears; compare with core `wp-login.php` behavior as the negative control.

### Authorization and credentials derived from the request itself

**PuppyFW through 0.4.4** [GHSA-f6x2-w8q2-ff47 / CVE-2026-88904](https://github.com/advisories/GHSA-f6x2-w8q2-ff47) guards a REST route by checking the caller against **a capability name taken from the request body**, so a subscriber trivially passes by naming a capability they hold, then adds/modifies/deletes arbitrary blog options → privilege escalation. Detection heuristic: any handler where the authorization *parameter* (capability, role, scope, org, tenant) is read from request material rather than hardcoded is instantly bypassable — enumerate handlers and diff the capability string source (hardcoded literal vs. `request[...]`). Prove with a subscriber-role session supplying its own held capability and one inert marker option; never mutate real options.

**wpShopGermany IT-RECHT KANZLEI before 2.4** [GHSA-xg7f-297r-82hv / CVE-2026-88795](https://github.com/advisories/GHSA-xg7f-297r-82hv) combines two credential-generation breaks: the API token is **derived from data the requester controls** and is **created as a side effect of the check that is supposed to validate it**. The validation endpoint is therefore also a minting endpoint, and the derived token grants arbitrary file write → RCE. Audit rule for any `check_token`/`verify`/`validate` style endpoint: does a failed or first-time check *generate* a credential? Is any input to the derivation attacker-influenced (site URL, email, timestamp, user ID)? Proof stops at prediction: derive the expected token offline in a lab, present it to a **recorder-patched** file-write sink, and record the denied marker path — never write real files.

**Newsletter before 9.3.8** [GHSA-w6v2-ggcj-c4hg / CVE-2026-86824](https://github.com/advisories/GHSA-w6v2-ggcj-c4hg) is the offline-key-recovery variant: the email-tracking link signing key lacks entropy and the links are signed with an **unkeyed hash**, so an attacker who recovers the key offline forges tracking links and obtains any subscriber's session token. When you see signed-but-unauthenticated-looking URLs in outbound mail (tracking, unsubscribe, preference links), check: (a) is the signature an HMAC with a per-install random secret, or a bare hash of predictable fields (site URL, user ID, timestamp)? (b) can you order two of your own lab accounts' links and interpolate the construction? This is the same purpose-binding lesson as the September 17 Craft HMAC page applied to email URLs. Proofs use your own two lab subscriber accounts only; never harvest real unsubscribe links.

### `is_admin()` is a request-context flag, not a role gate

**Paid Downloads through 3.15** [GHSA-8qvx-4h8h-jjwf / CVE-2026-87935](https://github.com/advisories/GHSA-8qvx-4h8h-jjwf) reaches its `admin_request_handler()` **unauthenticated** because `is_admin()` returns true for any `/wp-admin/admin-post.php` or `admin-ajax.php` request — the function reports *which script is running*, not *who is allowed*. This is the direct sibling of the Ai1wm `admin_init` lesson above and completes the wp-admin triad: `is_admin()` true, `admin_init` fired, and `PHP_AUTH_*` populated are all unauthenticated by default. Route-inventory rule: every WordPress plugin handler hooked or dispatched from `wp-admin/admin-post.php`, `admin-ajax.php`, or `admin_init` must carry its own `current_user_can()` + nonce check; treat the wp-admin URL prefix as a **public namespace** during recon and fuzz those endpoints with anonymous requests.

The same wave's **Multi Uploader for Gravity Forms through 1.1.9** [GHSA-h7vp-g8q2-89c8 / CVE-2026-87796](https://github.com/advisories/GHSA-h7vp-g8q2-89c8) adds the chunked-upload angle: file-type validation is applied per-chunk or on assembly metadata rather than the reassembled file, so an unauthenticated attacker uploads arbitrary files. When probing upload endpoints that accept `chunk`/`chunkIndex` parameters, re-test the assembled result separately from per-chunk behavior — MIME/extension checks that pass on every innocuous-looking chunk but never inspect the reassembled file are a recurring gap. Proof: benign marker content with an executable-looking extension into a disposable upload root only.

### Adjacent records from the same wave (processed without publication)

Payment-integrity axis (already covered by the Persian Elementor/Paid Membership sections): **Autopay before 5.0.1** unsigned payment callback disclosing/deleting stored payment parameters [GHSA-8cq7-r49f-2x26 / CVE-2026-90923](https://github.com/advisories/GHSA-8cq7-r49f-2x26) and **Paid Membership Subscriptions before 3.0.9** completing memberships without verifying provider-reported amount/currency [GHSA-3hfm-x4wx-j7vx / CVE-2026-90922](https://github.com/advisories/GHSA-3hfm-x4wx-j7vx) — both reuse the bind-provider-response-to-server-owned-price-state harness. Object-scope drift: **Motors before 1.4.121** unauthenticated draft/pending/private listing read by numeric user ID [GHSA-9r2r-hr84-wxp8 / CVE-2026-91016](https://github.com/advisories/GHSA-9r2r-hr84-wxp8); **Event Booking Manager before 5.3.8** unauthenticated attendee PII by enumerable booking reference [GHSA-vc5f-85m8-x67v / CVE-2026-91008](https://github.com/advisories/GHSA-vc5f-85m8-x67v) and **before 5.6.0** Contributor-readable PayPal/Stripe secret keys in stored gateway config [GHSA-2pgf-j495-ffj6 / CVE-2026-91019](https://github.com/advisories/GHSA-2pgf-j495-ffj6) (the low-role-stored-datasource-credentials axis; treat gateway options like the Eazy option-read pair above); **Comments Import & Export before 2.5.4** Author-role export of every site comment incl. unapproved content, emails, and IPs [GHSA-2fm8-39xv-9vv2 / CVE-2026-87836](https://github.com/advisories/GHSA-2fm8-39xv-9vv2); **LearnPress before 4.4.7** quiz answer-correctness flags returned to unauthenticated checkers on non-enrolled courses [GHSA-2848-wq6m-p5gf / CVE-2026-86446](https://github.com/advisories/GHSA-2848-wq6m-p5gf) (answer oracle, axis covered); **Invisible Anti-Spam before 5.1.1** tests nonce *presence* not validity [GHSA-rfgp-h9r2-p373 / CVE-2026-91010](https://github.com/advisories/GHSA-rfgp-h9r2-p373) (public-nonce axis covered); routine reflected/stored XSS singles (Realtyna Organic IDX, Dictionary, WPLP Cookie Consent, Dewa Kirim, EWWW, HT Mega, Master Addons public-nonce popup disable, Choose User Role registration role self-grant — the role-self-grant axis is already on this page).
...
## September 18 06:31Z follow-up: shared installation secrets, timestamp-derived tokens, and artifact-channel SSRF

The September 18 06:31Z WPScan-derived plugin wave adds five records with reusable axes; the WordPress **core** 7.1.1 release from the same window has its own page ([core parser, XML-RPC transport-parity, and object-scope boundaries](2026-09-18-wordpress-core-7-1-1-parser-xmlrpc-and-object-scope-boundaries-ghsa.md)).

- **All-in-One WP Migration and Backup before 7.111** [GHSA-9wwx-p46x-5vc7 / CVE-2026-81810](https://github.com/advisories/GHSA-9wwx-p46x-5vc7): several AJAX actions are gated only on an **installation-wide secret that is disclosed to any user permitted to export the site**, and carry no capability check — so a user with export-but-not-import permission can import an arbitrary site archive and take administrator. This completes the Ai1wm pair on this page: the September 17 record was an anonymous *write* into the plugin's option store; this one is a low-role *read of the plugin's own trust anchor*. Audit rule: for any plugin with export/import capability separation, ask whether the export path reveals the credential the import path accepts, and whether export and import capabilities are actually distinct. Non-default configuration (export granted below import) is a precondition — inventory role capabilities before claiming exposure. Prove on a disposable site: grant export-only to a canary user, retrieve the secret, and stop at a recorder-patched import sink; never import a real archive.
- **Easy Appointments before 4.0.2.2** [GHSA-w73j-m9cj-hp9q / CVE-2026-87965](https://github.com/advisories/GHSA-w73j-m9cj-hp9q): mail-link cancel/confirm authorization derives its token from a **hardcoded source-embedded salt plus the appointment's creation timestamp** — an entropy audit, not a bypass: reconstruct the construction in a lab, measure the effective candidate space over realistic timestamp windows, and confirm acceptance only against your own synthetic appointment. Sibling of the Fluent Forms receipt-hash section (observable-context-derived proofs); the timestamp input makes offline brute force practical. The companion record [GHSA-52p7-4rrr-jvcv / CVE-2026-87966](https://github.com/advisories/GHSA-52p7-4rrr-jvcv) (unauthenticated appointment overwrite/delete by request-supplied id) is plain missing-object-authorization on the reservation endpoint — same admin-ajax action sweep as the Motors item above.
- **Generate PDF using Contact Form 7 before 4.2.2** [GHSA-c3g6-9rgm-rg4m / CVE-2026-90984](https://github.com/advisories/GHSA-c3g6-9rgm-rg4m): the PDF renderer fetches image URLs embedded in submitted form content with no destination restriction, and the **fetched response comes back inside the generated PDF** — SSRF with a built-in response-artifact channel, no blind-oracle needed. When auditing any document-generation feature (PDF, DOCX, HTML email renderers, image optimizers), treat remote-resource fetchers inside the renderer as SSRF sinks whose output path is the generated artifact; prove with an owned no-content callback and record only that the callback was reached plus what the artifact embedded. Never target internal services.
- **VikBooking before 1.8.15** [GHSA-m65h-f32q-qmrr / CVE-2026-85127](https://github.com/advisories/GHSA-m65h-f32q-qmrr): unauthenticated live-chat attachments are neither type-validated nor content-sanitized, executing in the admin's browser when they open the conversation — anonymous-to-admin stored delivery through an attachment viewer, the same "attachment surface drops the validator" shape as AF Companion's import upload ([GHSA-7mpj-9p6h-7gr6 / CVE-2026-84738](https://github.com/advisories/GHSA-7mpj-9p6h-7gr6), low-role PHP upload → RCE) and the To Do List Member standalone upload endpoint. Recon rule: every chat/ticket/feedback attachment surface is an admin-viewer XSS or upload sink until the reassembled-file validator is proven; test assembled content, not per-chunk or filename metadata.
- **Filter Gallery before 1.1.5** [GHSA-hrf4-x4j2-f44x / CVE-2026-90978](https://github.com/advisories/GHSA-hrf4-x4j2-f44x): AJAX handlers verify the nonce **only when the nonce field is present** and apply no capability check — the parameter-ABSENT axis (twin of the Nextcloud Approval etag check and the null==null AVideo MFA bypass): always submit the control request with the security parameter omitted, not just forged. Low-privileged users can overwrite arbitrary post content and delete plugin options as a result.

## September 18 09:31Z follow-up: empty-policy fallthrough to admin, security-plugin fail-open, and validator-shape SSRF

The September 18 09:31Z WPScan-derived wave adds four records with reusable axes:

- **Booking Calendar through 11.8.2** [GHSA-jrfx-j7fq-9588 / CVE-2026-92619](https://github.com/advisories/GHSA-jrfx-j7fq-9588): the `wpbc_ajax_option_save` AJAX handler applies its `can_save`/`force_mode`/`allowed_keys` safeguards **only to option names explicitly registered** via `register_option_policy()`; any unregistered name — including core WordPress options — returns an *empty* policy and bypasses every check, so an Editor writes `default_role=administrator` + `users_can_register=1` via `update_option()` and self-registers an Administrator. The nonce gate is decorative: both the nonce value and the nonce action are attacker-supplied POST fields and a valid nonce is trivially fetched (`admin-ajax.php?action=rest-nonce`). Two audit rules: (1) **name-keyed policy registries fail open on unregistered names** — enumerate the writable key namespace and probe keys *outside* the registered set (sibling of the Grav `dynamicDataField` registered-handler bypass and the djust name-keyed safety grants); (2) any handler that reads its nonce *action* name from the request defeats the check by construction — treat `data_name`-style passthroughs into `update_option()` as global-authority sinks and prove on a disposable site with inert option values, never on a live blog.
- **Hide My WP Ghost before 7.0.11** [GHSA-5vw4-phcp-qmr8 / CVE-2026-86796](https://github.com/advisories/GHSA-5vw4-phcp-qmr8) + [GHSA-35qw-6f3x-m64f / CVE-2026-86800](https://github.com/advisories/GHSA-35qw-6f3x-m64f): the security plugin **disables its own firewall, threat-detection, and login/URL-hiding protections** when a request merely contains an attacker-suppliable WooCommerce-detection parameter, and drops URL hiding when the loopback security-check verification value is missing or wrong. This is the *protection-layer* inversion of the parameter-ABSENT axis: on hardening plugins, the fail-open trigger is often "verification absent/failed ⇒ assume legitimate and widen," so the recon move is to send the trigger parameter present-with-junk-value and absent in turn and diff what protection state the response reveals. When a target advertises a WP security plugin, treat the plugin itself as the first attack surface — its concealment state is a request-controlled toggle until proven otherwise.
- **Auto Upload Images through 3.3.2** [GHSA-7v2c-5xw3-f2hc / CVE-2026-12106](https://github.com/advisories/GHSA-7v2c-5xw3-f2hc): contributor-triggered image download uses `wp_remote_get()` (redirect-following, no address restrictions) with a `validate()` that rejects only URLs whose host equals the site's own hostname — private, loopback, and link-local (169.254.169.254) destinations pass. Audit rule for WP plugins: the validator *shape* is the finding — "reject own hostname only" is not an SSRF guard; check which WordPress HTTP API function the sink calls (`wp_remote_get` vs `wp_safe_remote_get`) and fuzz the `<img src>` ingestion path with owned-callback and lab-RFC1918 canaries only.
- **Clean Login before 1.19** [GHSA-whxq-7ph8-rv4q / CVE-2026-90977](https://github.com/advisories/GHSA-whxq-7ph8-rv4q) + [GHSA-vm4x-47jq-gqcc / CVE-2026-90976](https://github.com/advisories/GHSA-vm4x-47jq-gqcc): registration CAPTCHA is skipped when the stored session CAPTCHA value is **empty** (never initialize, and the control disappears), and the handler creates accounts without checking whether site registration is enabled. Both are the "control enforced only when state exists" family — on any CAPTCHA/MFA/state-bound gate, request the flow with the state deliberately absent (fresh session, cleared cookie) before trying to solve or replay it.

Same-wave records processed without publication (axes already covered on this page): Filter Gallery follow-up pair [GHSA-7pg5-g4gj-969x / CVE-2026-89413](https://github.com/advisories/GHSA-7pg5-g4gj-969x) (subscriber deletes arbitrary plugin records; explicitly confirms the nonce bypass requires *omitting* the field — present-but-invalid is rejected, sharpening the CVE-2026-90978 parameter-ABSENT entry above) and [GHSA-38vh-9c8v-rmgw / CVE-2026-89138](https://github.com/advisories/GHSA-38vh-9c8v-rmgw) (post-content overwrite + arbitrary `ufg_gallery_*` option writes); Mapster WP Maps through 1.23.0 arbitrary user-meta write with no nonce/capability/allowlist [GHSA-gxqq-92xq-fjh7 / CVE-2026-12954](https://github.com/advisories/GHSA-gxqq-92xq-fjh7) (object/meta-scope axis); generic stored-XSS/SQLi/IDOR singles (Complianz, ShopLentor, Qi Addons, Master Addons, CSS & JavaScript Toolbox, Strong Testimonials, Newsletter, NEX-Forms, VK All in One, Location Manager, WP Inventory Manager `where` clause, InfiniteWP `get_comments`, Printcart, Kubio, WCFM, Store Exporter, Download Manager IDOR, Custom Twitter Feeds, Foxtool, Magazine Blocks ×2, GPTranslate, Divi arbitrary shortcode execution, EmbedPress, Breeze cache-key tracking-param exclusion). Same-wave Synology DSM cluster (SCGI file read/write CVE-2026-13684, login entropy CVE-2026-13639, LDAP/CIFS/file-operation set) and HCL BigFix Service Management cluster (SQLi + cross-tenant PII CVE-2026-67100, search SSRF CVE-2026-67101, low-priv admin screens CVE-2026-67102) are advisory-summary-only with no function-level detail — tracked, not published; revisit if technical writeups land. TECHIN2B user-controlled-key auth bypass (CVE-2026-12384) is detail-free.

Adjacent records processed without publication (axes already covered on this page): Bookit appointment read/delete role-scope gaps [GHSA-qcfp-hh5m-fp23](https://github.com/advisories/GHSA-qcfp-hh5m-fp23)/[GHSA-2963-vmc7-jqxw](https://github.com/advisories/GHSA-2963-vmc7-jqxw); MasterStudy LMS cross-instructor student disclosure and REST order tampering [GHSA-g869-5rh7-8rvf](https://github.com/advisories/GHSA-g869-5rh7-8rvf)/[GHSA-332v-j356-q3mj](https://github.com/advisories/GHSA-332v-j356-q3mj); King Addons for Elementor template-import overwrite, image-action object drift, and private/draft post read [GHSA-49x6-35hm-mhc5](https://github.com/advisories/GHSA-49x6-35hm-mhc5)/[GHSA-pv3c-9qhj-4h2p](https://github.com/advisories/GHSA-pv3c-9qhj-4h2p)/[GHSA-gwpw-wwmj-xgqp](https://github.com/advisories/GHSA-gwpw-wwmj-xgqp) (status-state disclosure, object-scope, and content-overwrite axes); unauthenticated AJAX SQLi singles (Tz Weekly Radio Schedule ×2, Price Drop Alert, Product Q&A, wp shortcut link); stored-XSS/registration-bypass singles (iGMS, All Bootstrap Blocks tag-name position, RT Mega Menu ×2, Easy Form Builder ×2, Biggop, Really Simple Security unbounded option-key growth); RestroPress payment-recovery enumeration + cross-order note write; UpsellWP campaign-price cart abuse. ShortPixel Image Optimizer PHP object injection with no local POP chain [GHSA-mqfm-qx5g-96vc / CVE-2026-17086](https://github.com/advisories/GHSA-mqfm-qx5g-96vc) reuses the existing prove-deserialization-separately-from-gadget-availability harness. ManageEngine DataSecurity Plus unenrolled-agent authentication bypass and technician Reports SQLi (CVE-2026-18911/CVE-2026-18912) are tracked, not published — single-product, credential-gated, no new axis.

## September 19 03:32Z follow-up: conditional-nonce AJAX handlers as full-database readers

- **Divi Essential through 5.8.1** [GHSA-hcxp-7h6p-263r / CVE-2026-15760](https://github.com/advisories/GHSA-hcxp-7h6p-263r): the `dnxte_get_database_tables` and `dnxte_get_database_data` AJAX actions **verify a nonce only when the `nonce` POST parameter is present** (omit it and the check never runs) and never call `current_user_can()`. A Subscriber enumerates every database table and reads a caller-controlled row count from any table — `wp_users` (emails, password hashes), `wp_usermeta` (session tokens, secret keys), `wp_options` (API keys and credentials stored by other plugins). This is the purest instance on this page of the parameter-ABSENT axis (twins: Filter Gallery CVE-2026-89413 where present-but-invalid is rejected but absent is not, and Clean Login's empty-CAPTCHA skip): on any handler whose guard is `if ( isset( $param ) ) verify(...)`, the winning request is the one that **doesn't send the guard parameter at all**. Sweep AJAX actions for `isset($_POST['nonce'])`-style conditional checks with grep, then replay each action with and without the parameter from a Subscriber lab account against a synthetic marker row; never read real `wp_users`/`wp_usermeta` rows on a target — bound impact by table/column names, not collected values. Full wave context on the [Sept 19 shortcode/upload page](2026-09-19-wordpress-shortcode-execution-hidden-field-upload-and-encrypted-blob-dispatch-ghsa.md).
