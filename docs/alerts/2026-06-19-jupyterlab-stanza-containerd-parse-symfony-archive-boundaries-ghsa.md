...
## July 22 JupyterLab image and extension-policy follow-up

Four JupyterLab advisories add browser-render and package/plugin policy checks. The fixes are `jupyterlab` 4.5.10 and 4.6.2. Do not infer reachability from the package version alone: identify the image viewer, PyPI Extension Manager, plugin locks, or a downstream direct caller first.

| Advisory | Boundary to validate |
| --- | --- |
| [GHSA-gx64-gj6p-pc4c](https://github.com/advisories/GHSA-gx64-gj6p-pc4c) | A crafted image opened in the image viewer and then in a new browser tab could become active content in the JupyterLab origin. The advisory describes server-side code execution as a possible post-XSS impact, but the safe proof stops at an inert DOM marker. |
| [GHSA-89vp-jrxv-24w8](https://github.com/advisories/GHSA-89vp-jrxv-24w8) | Extension blocklist comparison used weaker normalization than PyPI, so equivalent names using dots, hyphens, underscores, or case changes could resolve to a blocked package. |
| [GHSA-h5v5-8746-g7mm](https://github.com/advisories/GHSA-h5v5-8746-g7mm) | Direct `/lab/api/plugins` requests could bypass administrator locks for child plugins or an extension-wide “lock all” rule. |
| [GHSA-whvh-wf3x-g77j](https://github.com/advisories/GHSA-whvh-wf3x-g77j) | A missing `await` skipped allowlist/blocklist enforcement for custom integrations calling `PyPIExtensionManager.install()` directly; stock HTTP handling performs a separate awaited check. |

For the image path, use a disposable JupyterLab profile with no real notebooks, tokens, terminals, or cloud credentials. Upload only a synthetic image fixture, open it through the normal viewer, then use the exact “open in new tab” action. Capture response headers, resulting origin, parsed DOM, and a harmless marker. Compare ordinary viewer display, direct file navigation, the new-tab action, and a fixed release. Do not call server APIs, open a terminal, or read browser/notebook state.

For extension policy, first confirm that kernels and terminals are unavailable or delegated; otherwise package installation may already be inside the user's authority. Seed a fake package index or use inert packages in an isolated environment. Compare canonical package names with case and `[-_.]+` variants, UI versus raw API plugin operations, parent-extension versus child-plugin locks, and stock HTTP handling versus a minimal downstream direct caller. Evidence should be an install/lock decision table and inert extension marker, never an untrusted public package or executable post-install behavior.

Report **image bytes to new-tab browser context**, **PyPI-equivalent package spelling to blocklist mismatch**, **child plugin/API operation to administrator lock bypass**, or **un-awaited direct install check to package-policy bypass**. Package installation is not a new privilege when the same user already controls a kernel or terminal; record that negative precondition explicitly.
