# TensorZero, ToolHive, local app, vault, NLP, and MCP documentation boundary checks

Source: hourly offensive-security scan, 2026-07-15 GitHub advisory wave, with July 16 NLTK follow-up. Primary entries: [GHSA-824w-x939-6cmc](https://github.com/advisories/GHSA-824w-x939-6cmc), [GHSA-pr64-jmmf-jp54](https://github.com/advisories/GHSA-pr64-jmmf-jp54), [GHSA-q78p-hj9h-5466](https://github.com/advisories/GHSA-q78p-hj9h-5466), [GHSA-62gx-5q78-wrvx](https://github.com/advisories/GHSA-62gx-5q78-wrvx), [GHSA-pph6-vfjv-vpjw](https://github.com/advisories/GHSA-pph6-vfjv-vpjw), [GHSA-xgch-x3mx-cm3c](https://github.com/advisories/GHSA-xgch-x3mx-cm3c), [GHSA-f5pf-q7c7-m3vv](https://github.com/advisories/GHSA-f5pf-q7c7-m3vv), [GHSA-6f5r-5672-72j7](https://github.com/advisories/GHSA-6f5r-5672-72j7), and [GHSA-p4gq-832x-fm9v](https://github.com/advisories/GHSA-p4gq-832x-fm9v) / CVE-2026-54293.

This batch is durable because the advisories expose repeatable operator boundaries: user-controlled object-storage configuration crossing into gateway file reads and outbound fetches, remote MCP discovery responses steering host-side authentication metadata fetches outside ToolHive's container sandbox, browser-readable local developer apps through wildcard CORS, local vault APIs and NLP resource loaders that decode path separators after earlier validation, SSRF guards that omit IPv6 transition or special-purpose ranges, downloader crawler selection that treats host substrings as trusted service identity, and documentation-server web UIs that bind to all interfaces with unauthenticated corpus administration APIs.

!!! warning "Authorized validation only"
    Keep proofs to disposable TensorZero gateways, ToolHive and MCP documentation-server labs, local FiftyOne/Obsidian/NLTK labs, owned browser origins, synthetic vaults, owned callback domains, fake object stores, canary files, fake API keys, and isolated IPv6/NAT64 test networks. Do not read real credentials, notebooks, datasets, Obsidian notes, application configs, cloud metadata, customer media, production object stores, browser profile data, live downloader API keys, real MCP documentation corpora, or host-side environment variables. Do not write or delete files outside disposable lab canary paths.
...
| [GHSA-6f5r-5672-72j7](https://github.com/advisories/GHSA-6f5r-5672-72j7) / CVE-2026-54504 | `@andrea9293/mcp-documentation-server` Web UI/API | Version `1.13.0` starts the default Web UI on all interfaces (`0.0.0.0:3080`) and exposes document-management endpoints without authentication; `1.13.1` is listed as the first patched version | Add default-bind and unauthenticated corpus-admin checks to MCP documentation/search helpers. Network-local exposure can turn a developer convenience UI into document read/write/delete/search access. |
| [GHSA-p4gq-832x-fm9v](https://github.com/advisories/GHSA-p4gq-832x-fm9v) / CVE-2026-54293 | NLTK `nltk.data.load()` resource loader | `nltk:` resource strings are checked for unsafe paths before URL decoding; `%2f` and `%2e` variants can become absolute or traversal paths after `url2pathname()` | Treat AI/NLP resource identifiers as file-read sinks. Any app, notebook service, ML pipeline, or CI helper that passes user-controlled corpus/model/resource names into NLTK needs decode-order canary tests. |
...
1. Install `@andrea9293/mcp-documentation-server` `1.13.0` in a disposable lab with `MCP_BASE_DIR` pointing to a temporary directory containing only synthetic documents. Use `1.13.1` or later as the patched negative control when available.
...
### NLTK `nltk.data.load()` encoded-traversal checks

1. Identify whether the target accepts corpus, tokenizer, model, or other NLTK resource identifiers from users, uploaded notebooks, workflow configs, API parameters, queue jobs, or CI variables and passes them into `nltk.data.load()` or a wrapper that preserves the `nltk:` URL scheme.
2. Reproduce only in a disposable lab with NLTK `<= 3.9.4`, an empty synthetic `nltk_data` directory, and one canary file in a temporary sibling directory outside the intended NLTK data roots.
3. Establish controls first: a legitimate `nltk:` resource lookup, a literal `../` traversal that should be rejected, and a nonexistent canary path.
4. Replay URL-encoded separator variants that target only the synthetic canary, such as encoded leading separators or `..%2f` / `%2e%2e%2f` segments. Preserve encoding on the wire or in the job config so the application, not the client library, performs the decode-after-check sequence.
5. Positive evidence is limited to reading the marker canary through the NLTK loader while the literal traversal control is blocked. Do not target `/etc/passwd`, `/proc/self/environ`, SSH keys, notebooks, application source, cloud config, or service-account material.
6. Add controls for NLTK `3.10.0` or later, strict resource allowlists, decoding before validation, canonical path containment checks after decoding, disabled `nltk:` URL inputs, and non-user-controlled resource names.

Report this as **user-controlled NLTK resource URL -> validation before URL decoding -> encoded traversal reaches outside NLTK data roots**. Evidence should be version, resource input source, encoded path class, literal-traversal negative control, synthetic marker read, and patched-version negative control.

## Reporting notes

- Lead with preconditions: exposed TensorZero gateway access, ToolHive remote MCP trust state, MCP documentation-server Web UI settings, local FiftyOne/Obsidian server state, NLTK resource-name control, browser origin, API-key/auth state, IPv6/NAT64 network assumptions, and downloader credential configuration.
...
- Redact API keys, callback tokens, local file paths beyond synthetic temp directories, browser origin secrets, object-storage credentials, vault names, NLTK corpus paths, interface names, document corpus contents beyond markers, and any request body field that would reveal production topology.
