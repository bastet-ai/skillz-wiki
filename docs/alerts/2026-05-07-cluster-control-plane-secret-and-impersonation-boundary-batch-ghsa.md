...
- **Rancher Extensions path traversal** — [GHSA-5v3h-x4wf-5c35](https://github.com/advisories/GHSA-5v3h-x4wf-5c35): extension file-serving needs canonical path enforcement and package-root containment.
...
4. Inventory Rancher extension packages and remove unknown, writable, or tenant-supplied extension roots.
5. On Windows ECS hosts, review agent exposure, local access paths, and logs for unexpected metadata reads.
...
- Canonicalize extension file paths after decoding and symlink resolution, then verify they remain under immutable package roots.
