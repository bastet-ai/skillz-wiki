...
## September 16 late-wave LiteLLM follow-up: MCP spec loader SSRF, async guardrail `call_type` mismatch, SSO session retention, user-enumeration fix gap (4 GHSAs)

Four additional records (published 2026-06-21, surfaced via the updated feed 2026-09-14) extend the LiteLLM boundary set. All affect ≤ 1.82.x lines; confirm the deployment version and route before validating.

| Advisory | Boundary to validate |
| --- | --- |
| [GHSA-c693-x898-5g4h](https://github.com/advisories/GHSA-c693-x898-5g4h) / CVE-2026-12798 | MCP OpenAPI spec loader `load_openapi_spec_async` accepts an unvalidated `spec_path` URL → server-side fetch of arbitrary destinations (SSRF). Same loader pattern as the July 22 MCP OAuth entry: an MCP-config field is an egress control point. |
| [GHSA-p897-vf7j-f5h8](https://github.com/advisories/GHSA-p897-vf7j-f5h8) / CVE-2026-12797 | BannedKeywords / AzureContentSafety guardrails bypass via `call_type` mismatch on async endpoints — the pre-call hook keys off a call-type field that async routes set differently, so the guardrail silently no-ops. Guardrail coverage must be tested per endpoint family (sync, async, streaming), never assumed from one passing test. |
| [GHSA-w2mh-qq9q-453x](https://github.com/advisories/GHSA-w2mh-qq9q-453x) / CVE-2026-12796 | SSO login does not invalidate previous UI session tokens — re-authenticating via SSO leaves old session tokens live. Session-rotation differential: capture token A, SSO re-login, confirm token A still authorizes. |
| [GHSA-m2v5-74w2-qhcj](https://github.com/advisories/GHSA-m2v5-74w2-qhcj) / CVE-2026-12799 | `ui_view_users` incomplete fix for CVE-2025-0628 → UI user enumeration still yields system-wide disclosure. **Incomplete-fix follow-up pattern:** re-run the original enumeration matrix on the fixed build rather than trusting the advisory history. |

Validation stays inside the July 22 discipline: disposable proxy with fake provider keys, owned no-content callback for the `spec_path` leg, benign marker keyword for the async guardrail leg (compare sync vs async endpoint outcomes), and two disposable UI sessions for the rotation check. Never enumerate real users, never point `spec_path` at internal services or metadata endpoints, never retain real session tokens in evidence. The reusable axes — **per-endpoint guardrail parity**, **SSO re-auth session invalidation**, **config-field SSRF on MCP loaders** — are already the page's audit checklist; these four are added rows, not a new page.

## September 17 follow-up: `user_config` nesting defeats the `api_base` SSRF guard (GHSA-hx8v-g79f-8w5f)

[GHSA-hx8v-g79f-8w5f](https://github.com/advisories/GHSA-hx8v-g79f-8w5f) / [CVE-2026-59823](https://nvd.nist.gov/vuln/detail/CVE-2026-59823) (affected ≤ 1.83.8, fixed 1.83.9): `is_request_body_safe` blocks top-level `api_base` and `base_url` keys, but the `user_config` request-body object is also used to build the outbound router — smuggling `api_base` **inside `user_config`** redirects the proxy's outbound request to any host. Exploitation requires a valid virtual key (authenticated-caller SSRF).

Operator value: the guard is a **flat key-blocklist applied only at the top level of a nested JSON body that is itself merged into router construction**. The audit rule generalizes to any request body with config-object passthrough: enumerate which nested objects reach outbound-request construction (`user_config`, `litellm_params`, `deployment`, model config) and re-test every blocked top-level key at each nesting depth — blocklists that do not walk the tree are bypassed by one level of nesting. Validation: disposable proxy with fake provider keys, one owned no-content callback as the nested `api_base` target, one virtual key; evidence is the callback hit with the redacted key attached. Never point at internal services or metadata endpoints. Same shape as the `spec_path` MCP-loader row above — a body field is the egress control point — but this one proves the depth gap, not the missing check.
