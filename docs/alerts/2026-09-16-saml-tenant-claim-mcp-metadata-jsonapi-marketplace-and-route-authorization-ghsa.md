...
## September 16 late-wave follow-up: Concrete CMS authorization quartet (4 GHSAs)

The 2026-09-16T18:32Z unreviewed wave carried four Concrete CMS records that share the wiki's **alternate-route and token-lifecycle authorization** axes:

| GHSA | Sev. | Boundary |
| --- | --- | --- |
| [GHSA-f2mp-hj9g-qf89](https://github.com/advisories/GHSA-f2mp-hj9g-qf89) | high | < 9.5.4: public Form Block file-upload question accepted XML/XSLT with **extension-only validation**, stored files publicly and served them **inline from the application origin**. A stored XML document with an `xml-stylesheet` PI referencing an attacker-supplied same-origin XSLT transforms to HTML and executes attacker JS in the CMS origin when a victim opens the stored file (stored XSS); an admin victim yields full admin actions (reporter demonstrated creating an Administrators-group user). |
| [GHSA-3w9f-9wfp-x8gr](https://github.com/advisories/GHSA-3w9f-9wfp-x8gr) | medium | < 9.5.3: legacy Express entry search endpoint returned entry JSON **without the `canViewExpressEntries()` check** applied by the dashboard/CSV-export flow; entities that don't support entry-specific permissions also get `ignorePermissions()` → unauthenticated attribute disclosure by entity ID. |
| [GHSA-82fx-23wg-996j](https://github.com/advisories/GHSA-82fx-23wg-996j) | low | 9.2.0–9.5.3: `POST /ccm/api/1.0/users` performed **no permission check before creating accounts** — any OAuth token with `users:add` scope, *including a client_credentials token with no user context*, created active validated accounts bypassing email verification; default settings then grant page-edit (stored XSS path). |
| [GHSA-pmgv-hgv2-w69h](https://github.com/advisories/GHSA-pmgv-hgv2-w69h) | low | < 9.5.4: REST resource server authorized from the **bearer token alone** — existence + expiry + explicit-revocation only. Deactivating, deleting, or locking an account did not invalidate its outstanding tokens → full `/ccm/api/1.0/*` access persists for token lifetime. |

Operator patterns to reuse:

- **Public upload that serves inline from app origin = stored XSS vector even without HTML.** XML+XSLT (`xml-stylesheet` PI) is the classic non-HTML script-execution pair: test every "file upload, publicly served" surface for (a) extension-only MIME checks, (b) inline `Content-Type` serving, (c) whether the stylesheet/reference must be same-origin — here the attacker just uploaded the XSLT too.
- **Legacy/alternate endpoints drop the permission call.** The Express entry search route skipped the exact check its dashboard and export siblings invoke — the recurring wiki "enumerate route families, diff the checks" workflow; also note `supportsEntrySpecificPermissions() == false` silently widening to `ignorePermissions()`.
- **Scope-only OAuth authorization with machine tokens.** A `client_credentials` token has no user behind it; any admin-capable scope (`users:add`) on such a token converts an app credential leak into account minting. Audit scope→action maps for user-context assumptions.
- **Token lifecycle vs account lifecycle.** Any bearer-only API must re-check account state (deactivated/deleted/locked) at request time; test by issuing a token, deactivating the user, and replaying — a complete finding with a disposable account.
...
- GitHub Advisory Database: Concrete CMS [GHSA-f2mp-hj9g-qf89](https://github.com/advisories/GHSA-f2mp-hj9g-qf89), [GHSA-3w9f-9wfp-x8gr](https://github.com/advisories/GHSA-3w9f-9wfp-x8gr), [GHSA-82fx-23wg-996j](https://github.com/advisories/GHSA-82fx-23wg-996j), [GHSA-pmgv-hgv2-w69h](https://github.com/advisories/GHSA-pmgv-hgv2-w69h)
