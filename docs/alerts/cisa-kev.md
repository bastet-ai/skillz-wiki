...
- **2026-09-18 KEV: Linux kernel pair — tracked, no operator page.** CISA added **CVE-2025-39964** (`crypto: af_alg` concurrent-write socket-state inconsistency) and **CVE-2026-53266** (`netfilter: bridge` ebt SNAT ARP sender-HW-address rewrite past a non-writable header region, OOB write) on 2026-09-18 (catalog 2026.09.18, 1715 entries). Both are local kernel primitives with no documented public exploit path or remote attack surface; relevant to post-exploitation planning on compromised Linux hosts (which kernel build still has a local-write primitive) rather than a standalone operator page. `af_alg` remains a durable container-escape/local-attack interest area; the netfilter item matters only where bridge-netfilter is loaded in the target's kernel.
