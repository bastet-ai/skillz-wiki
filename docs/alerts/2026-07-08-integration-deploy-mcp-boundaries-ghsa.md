...
**Third variant (Sept 22 fold, [GHSA-5gpm-rgj3-9q76](https://github.com/advisories/GHSA-5gpm-rgj3-9q76) / [CVE-2026-86043](https://nvd.nist.gov/vuln/detail/CVE-2026-86043), Skipper ≤0.27.33, current HEAD):** the published *mitigation* for the second variant is itself bypassable. Advisory GHSA-8qqm-fp2q-v734 told policy authors to guard on `input.truncated_body` — but the `truncated_body` flag is computed by the OPA envoy plugin **only when a `content-length` header is present**. `Transfer-Encoding: chunked` (HTTP/1.1) and HTTP/2 requests carry no content-length, so oversized bodies arrive at the policy with `truncated_body` **absent** (≠ `false`), and the recommended `input.truncated_body == false` guard fails open exactly on the transports the original bypass used. Operator lesson for every "guard on flag X" remediation: test the guard's behavior when the flag is **absent vs false vs true** — a deny-on-`false` policy treats an uncomputed flag as not-false, and mitigation guidance is an attack map once you know which field it asks you to trust. Sweep the whole fix chain (GHSA-659f-rgp5-w4wf → GHSA-8qqm-fp2q-v734 → this) as three attempts to close one policy/parser gap; the framing dimension (fixed-length vs chunked vs H2) is the branch that keeps getting missed. Validation stays per the three-control protocol above; add the fourth control: oversized chunked body **with** the vendor-recommended Rego guard in place — that is the fail-open proof.
