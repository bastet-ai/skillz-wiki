...
## September 21 follow-up: DNS-pin reuse SSRF, anonymous user-enumeration oracle, CSRF-scope and destination-side authz drift (8 GHSAs, Concrete CMS ≤ 9.5.2 → 9.5.3)

A fresh Concrete CMS cluster published 2026-09-21T18:32Z adds three reusable axes to this page.

- **[GHSA-fmfq-8qcq-wrx4](https://github.com/advisories/GHSA-fmfq-8qcq-wrx4) / CVE-2026-18424 — remote file import SSRF via cross-port reuse of a validated DNS pin.** When several remote URLs share a host, only the first `ValidatedRemoteUrl` is retained and reused for every later URL with that host; the unpinned download leg then follows a DNS-rebinding name that resolved public at validation time and private at fetch time, saving loopback/metadata responses into the file manager. Durable rules: **a validation-time DNS pin is only sound if the resolved IP is pinned for the actual connection, per-URL and per-port — "validated once per host" is a TOCTOU; and file-import features that *save* the fetched bytes are non-blind SSRF read channels** (the saved file is the response oracle). Two-URL same-host battery (public first, rebinding second) on any importer with owned DNS records.
- **[GHSA-7hmf-f7jv-m4xw](https://github.com/advisories/GHSA-7hmf-f7jv-m4xw) / CVE-2026-18110 — unauthenticated full backend-account enumeration via `/ccm/system/user/autocomplete`.** The selector's "access token" is bound to display options, not the caller, and is handed to anonymous visitors; empty query = match-all filter, paginated → every user's ID, username, and email including super-admin. Durable rules: **UI-component CSRF-style tokens minted anonymously are not authorization; test every selector/picker/autocomplete endpoint with the token from an anonymous page render and an empty query**; and user-enumeration oracles are the precondition artifact for password-spray/reset-flow campaigns — report as an identity-surface finding with role breakdown, not just "info disclosure".
- **Authorization-scope drift octet:** Express Form block control-management actions rely on a CSRF token **bound to user+action but not to the specific block/page/form** → token from your own form authorizes control edits on foreign forms, incl. HTML-rendered control = stored XSS ([GHSA-g5mv-jcqh-ch66](https://github.com/advisories/GHSA-g5mv-jcqh-ch66) / CVE-2026-18426); sitemap reorder checks only the global `access_sitemap` task permission, no per-page edit check, no CSRF token ([GHSA-rvm5-mg98-27w7](https://github.com/advisories/GHSA-rvm5-mg98-27w7) / CVE-2026-18425); multilingual page assignment checks source-side permission only — **no destination-side authorization check**, bind/delete foreign-locale translation pairs ([GHSA-hch9-r5fg-6p72](https://github.com/advisories/GHSA-hch9-r5fg-6p72) / CVE-2026-18422); Express saved-search preset delete/edit gated only on view permission of *any* entity ([GHSA-7vc7-m9pm-w64f](https://github.com/advisories/GHSA-7vc7-m9pm-w64f) / CVE-2026-18423). Sweep rules: **for every token, ask what object it is bound to (user+action ≠ user+object); for every relationship write (assign/reparent/share), check authorization on BOTH endpoints; and run the low-role IDOR sweep over admin *sub*-dialogs, not just list routes.**
- **Render-sink pair:** colliding page paths echoed unmodified by the check endpoint and inserted as raw HTML by the client into the duplicate-path dialog ([GHSA-994m-pc89-jhp9](https://github.com/advisories/GHSA-994m-pc89-jhp9) / CVE-2026-81926) — API check-echo → client `innerHTML` is a two-leg chain where neither leg's author notices the other; and user-supplied custom date format rendered into conversation messages, delivered via **CSRF-less auto-submit POST** ([GHSA-3mw6-rrq3-376v](https://github.com/advisories/GHSA-3mw6-rrq3-376v) / CVE-2026-81925) — format strings are code to a date library, and "POST-only" endpoints without tokens are still deliverable cross-origin.

Proof boundaries unchanged: disposable CMS installs, two lab roles (low-priv editor vs foreign owner), owned DNS records with short-TTL rebinding to a lab canary port, synthetic accounts/presets/translations, inert structural XSS markers. Never enumerate real accounts on a client target without written scope, and never import production-reachable metadata endpoints.
