...
## September 20 follow-up: PROXY-protocol trust extends to native-parser memory safety

The SMTP PROXY boundary on this page is usually framed around Go daemons and source-IP policy. The September 20 Exim wave shows the same trust relationship carrying **memory-safety risk into the MTA itself**: [GHSA-xcfp-8rjq-j39w](https://github.com/advisories/GHSA-xcfp-8rjq-j39w) / CVE-2026-94056 (7.5, uninitialized stack-memory read) and [GHSA-r24f-7cqq-88w4](https://github.com/advisories/GHSA-r24f-7cqq-88w4) / CVE-2026-94054 (7.0, out-of-bounds write) in Exim before 4.100.1, both reachable when Proxy-Protocol is used **with an attacker-controlled proxy**. The companion [GHSA-3mrg-v8x6-xfjw](https://github.com/advisories/GHSA-3mrg-v8x6-xfjw) / CVE-2026-94057 message-smuggling record is promoted on the [September 20 parser page](2026-09-20-exim-smtp-rejection-smuggling-and-expat-surrogate-markup-hiding-ghsa.md).

Operator value: when an authorized assessment establishes that an MTA accepts PROXY protocol from a network position you control (per the double-PROXY checks above), the review scope now includes the native PROXY-line parser as an attack surface, not just the policy layer that consumes the rewritten IP. Fingerprint the MTA (banner, `exim -bV` where local, headerReceived-line style) and version-check before probing; memory-safety validation stays lab-only on your own instance.
