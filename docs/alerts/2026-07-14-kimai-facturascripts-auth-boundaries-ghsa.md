...
| [GHSA-hgjx-r89m-m7v4](https://github.com/advisories/GHSA-hgjx-r89m-m7v4) | FacturaScripts `UploadedFile::move()` call sites | Raw `getClientOriginalName()` is concatenated to the destination path for API, document, widget, library, and plugin uploads | Add client-filename traversal checks to ERP/CMS upload features and separate arbitrary write from execution claims. |
...
### FacturaScripts upload filename path traversal

1. In a disposable FacturaScripts lab, create a non-admin account with only the upload capability under assessment and seed temporary marker directories under `MyFiles/`, `Dinamic/Assets/`, and another writable non-served temp path.
2. Upload a harmless text or image canary through each reachable upload route family (`/api/3/uploadfiles`, `/api/3/attachedfiles`, file widgets, library widgets, document-file traits, and plugin zip upload where roles permit).
3. Set the multipart `filename` to a traversal-shaped value that should be rejected or basename-normalized, aiming only at a disposable marker path outside `MyFiles/`.
4. Verify whether the server writes the marker at the final resolved path and whether the stored metadata records the raw filename. Do not write `.htaccess`, PHP, plugin code, shell scripts, cron files, startup files, or existing application assets.
5. If assessing direct web exposure, prove only the static-serving boundary with inert marker bytes in a lab path; do not publish or attempt executable content, and do not claim RCE unless a separately authorized lab reproduces execution safely.
6. Add controls for patched builds, sanitized basenames, blocked extensions, non-writable destinations, and routes that independently rename files.

Report this as **multipart filename -> unnormalized destination join -> file write outside intended upload root**. Evidence should be filename grammar, final-path marker existence, process owner, route/role matrix, and patched negative controls.
...
- Prefer decision tables over payload dumps: route, method, actor role, submitted object ID, expected authorization predicate, observed status, marker effect, authentication step, file-root decision, API filter-key grammar, upload filename grammar, final resolved path class, service-key write-marker state, and patched control.
- Redact cookies, signed URLs, login links, reset tokens, TOTP seeds/codes, `APP_SECRET` values, customer names, billing rates, export-template bodies from real tenants, user IDs from real tenants, invoices, attachments, backup contents, password hashes, `logkey` values, API keys, cloud service-key bodies, purchase/JWS payloads, OCSP/certificate material tied to production systems, and any uploaded file content not created solely as a canary.
