...
| [GHSA-78mf-482w-62qj](https://github.com/advisories/GHSA-78mf-482w-62qj) / CVE-2026-34403 | `github.com/0xJacky/Nginx-UI` | All authenticated WebSocket endpoints accept arbitrary `Origin`, so cookie-backed admin sessions can be reused by attacker pages for CSWSH; see the [dedicated note](2026-05-07-nginx-ui-websocket-origin-boundary-ghsa-78mf-482w-62qj.md). | Fixed package metadata: `1.9.10-0.20260316053337-1a9cd29a3082`; vendor release guidance points to 2.3.5+ |
...
- **Nginx-UI:** find exposed or recently restarted fresh installs, especially Docker/LAN/test instances. Upgrade to 2.3.8+ where covered, rotate JWT/node/OIDC/Casdoor secrets, and inspect restore/install logs for first-run claims or backup uploads. Also review WebSocket upgrade logs for unexpected `Origin` values against `/api/pty`, `/api/nginx_log`, `/api/events`, analytics, upgrade, and cluster endpoints.
