...
| [GHSA-h4pc-58cc-hc95](https://github.com/advisories/GHSA-h4pc-58cc-hc95) | Apollo ConfigService raw config endpoint | `/configfiles/raw/{appId}/{clusterName}/{namespace}` authentication parses `appId` as literal `raw` while downstream handling resolves the protected app ID | Add path-segment parser-differential checks to signed config-file endpoints where authentication and resource lookup use different route models. |
| [GHSA-4w3q-qpfq-v992](https://github.com/advisories/GHSA-4w3q-qpfq-v992) | Apollo ConfigService app ID matching | Authentication accepts a non-canonical app ID variant that misses the AccessKey cache while downstream request handling resolves to the protected app | Test canonicalization gaps where signature enforcement, AccessKey lookup, and config retrieval do not normalize app IDs identically. |
...
2. For the raw config route, send a legitimate signed request to `/configfiles/raw/{appId}/{clusterName}/{namespace}` and record the expected signature/app ID decision.
3. Send the raw-route variant that causes the authentication layer to derive `appId=raw` while the downstream config-file handler resolves the real `{appId}` segment. If no AccessKey exists for an app named `raw`, confirm whether the protected app config is returned without a valid protected-app signature.
4. For non-canonical app ID matching, test only harmless variants that the downstream resolver treats as the protected app while the AccessKey lookup cache does not. Capture a decision table: request path/app ID variant, key looked up, signature required, config app returned, and patched result.
5. Include controls for a correctly signed protected-app request, an unsigned canonical protected-app request, a nonexistent app, an app literally named `raw` if present in the lab, and patched builds where auth and resource lookup share the same canonical app ID.

Report this as **request path/app ID variant -> AccessKey lookup on different identity -> downstream protected config returned**. Use synthetic app IDs and config markers only; never include real namespaces, secrets, service URLs, or production AccessKey material.
