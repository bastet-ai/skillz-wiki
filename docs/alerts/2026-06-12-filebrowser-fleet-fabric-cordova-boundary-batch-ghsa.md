...
1. **Start from role and rendering preconditions.** File Browser checks vary by primitive: command allowlist bypass requires command execution to be re-enabled, a configured shell interpreter, Execute permission, and at least one allowed command; future-share/prefix/symlink/archive planting generally require authenticated share, delete, create, write, or existing symlink conditions; public directory rule rebasing requires only knowledge of a public directory share URL once a share exists. Fleet requires Observer credentials and affected endpoints; Fabric.js requires an application that accepts attacker-controlled canvas/gradient data and renders exported SVG into the DOM; Cordova requires a target app that opens attacker-controlled or interceptable content in InAppBrowser on iOS.
2. **Prove the boundary with canaries, not secrets.** Use inert command markers, disposable files, synthetic share links, lab deny rules, harmless archive marker filenames, symlinks to non-sensitive marker files, test Fleet hosts with canary enrollment values, harmless SVG markers, and inert Cordova callback payloads. Do not run production commands, read real user files, recover production enrollment secrets, delete live share links, overwrite out-of-scope files, or target real mobile users.
...
### File Browser command allowlist shell validation

- Use only a lab File Browser instance or a tenant where the owner explicitly approved command-execution testing. The preconditions are narrow: command execution is enabled, a shell interpreter is configured, and the test user has Execute permission plus at least one allowed command.
- Choose one harmless allowed command such as `printf`, `echo`, or `true` and pair it with an inert marker command that writes only to a disposable temp path or prints a canary string. Do not use commands that read system files, enumerate users, open sockets, or persist changes.
- Open the approved command API/WebSocket path as the disposable user and submit two controls: the marker command alone should be rejected by the allowlist, while the allowed command plus a shell metacharacter and the same marker should execute if the boundary is vulnerable.
- Positive proof is the mismatch between allowlist rejection for the raw marker and execution when the marker is chained after an allowed first token through the configured shell.
- Evidence: File Browser version, command-execution setting, configured shell value, disposable user permissions, allowed command list, redacted request transcript, canary-only output, and patched/default-disabled behavior in v2.33.8 or later.
...
- Name the exact crossed boundary in the report title: `command allowlist first-token shell bypass`, `future share grant to later file exposure`, `logical path prefix to cross-user share deletion`, `public directory share rule rebasing`, `archive entry separator traversal on Windows extraction`, `scoped-root symlink follow`, `ORDER BY cursor oracle to enrollment-key leakage`, `canvas data to trusted SVG DOM`, or `InAppBrowser web content to native callback dispatch`.
- Keep impact tied to demonstrated permissions and sinks. These are high-signal when the report preserves the role, route, platform, filesystem setup, extractor, symlink precondition, shell configuration, and rendering requirements instead of claiming blanket file disclosure, database exfiltration, arbitrary host write, or mobile RCE.
- Redact share hashes, enrollment secrets, mobile device identifiers, APNS material, plugin callback data, user files, command output beyond the canary, and filesystem paths that reveal tenant names or sensitive directory layouts. Synthetic canaries are sufficient.

## Notes on skipped adjacent items

The same scan rechecked Disclosed, PortSwigger, Trail of Bits, ProjectDiscovery, GitHub advisory published/updated feeds, and CISA KEV. ConnectBot DER/SSH memory-allocation items, PyO3 memory-safety issues, File Browser public-login resource exhaustion, Jenkins stored XSS, Apostrophe updated rich-text/password/XSS/CLI items, Docker `cp` race updates, and generic resource-consumption updates were tracked but not promoted because they did not add a clearer replayable offensive workflow than the command, share, oracle, serialization, filesystem-boundary, and mobile bridge boundaries above. No new PortSwigger, Trail of Bits, ProjectDiscovery, Disclosed, or CISA KEV item in this hour added additional durable guidance.
