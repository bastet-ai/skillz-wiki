...
## September 16 follow-up: changedetection.io browser-step Goto SSRF and browserless file-protocol non-enforcement

Two more fetch-worker advisories extend this page's browser-automation boundary axes (published 2026-09-16T21:32Z):

- **changedetection.io browser-step Goto SSRF** — [GHSA-4jg2-g9g4-7fpp](https://github.com/advisories/GHSA-4jg2-g9g4-7fpp) / CVE-2026-92815: through 0.60.6, the **Goto URL action in browser steps** never validates `optional_value`, so an attacker who can define/modify watch browser steps supplies arbitrary internal URLs and retrieves responses from restricted network locations. Validation note: the page-watch URL may be filtered while the *step-action* URL is a separate sink — enumerate every URL parameter a watch config accepts, not just the primary target field. Proof: lab instance, watch step Goto to an owned callback URL, capture the outbound hit; never probe real internal addresses.
- **browserless `file:` navigation despite `ALLOW_FILE_PROTOCOL=false`** — [GHSA-w2m9-jxm7-c4jp](https://github.com/advisories/GHSA-w2m9-jxm7-c4jp) / CVE-2026-92811: versions 1.44.0–2.56.7 fail to enforce the file-protocol restriction on the **Playwright websocket endpoints** (the CDP/Playwright wire path), even though the env default is deny. A token holder navigates the remote browser to `file://` URLs and reads any file the container process can. This is the protocol-guard parity pattern again: the HTTP navigation path enforced the flag, the websocket control path did not. Replayable check on an owned browserless instance: connect via Playwright over websocket, `goto("file:///proc/self/environ")`-adjacent lab marker file you created, compare against the HTTP-API verdict. Reading your own marker is the proof; never read app config or secret files.

Operator sweep update: for any hosted browser-automation service (Crawl4AI, browserless, Playwright/CDP grids), the protocol/URL guard must be verified **per transport** (REST endpoint vs websocket/CDP session) and **per URL-bearing parameter** (target URL vs step/action URLs). Same reporting discipline as above: marker content only, own callbacks only.
