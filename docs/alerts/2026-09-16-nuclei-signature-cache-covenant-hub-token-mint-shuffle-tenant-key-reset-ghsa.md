...
## September 22 follow-up: the v3.10.0/v3.11.0 Nuclei template-sandbox wave — five more ways untrusted templates escape the per-protocol gates

The scanner itself keeps being the softest target in the toolchain (this page's mtime-cache bypass is leg zero). The July-published, Sept-22-GHSA-database-synced Nuclei wave adds five legs, all on the standing rule that **Nuclei's per-protocol gates (`-code`, `-file`, `-lfa`, code signing) are enforced per loader branch, and untrusted templates flow through the branches that skip them**:

- **DAST code-signature bypass** (CVE-2026-76802, [GHSA-jpf4-98qj-qr67](https://github.com/advisories/GHSA-jpf4-98qj-qr67)): a template combining a `fuzzing:` block (DAST-eligible) with an unsigned `code:` block was routed through the DAST loader branch, which never enforced the code-template signature check — arbitrary shell execution from an unsigned `code:` block with only `--dast` set (fixed 3.10.0). Audit shape: enumerate loader branches and diff which gates each one enforces; a gate present in the main branch proves nothing about sibling branches.
- **Workflow file-protocol gate bypass** (CVE-2026-76804, [GHSA-qgw5-7j4f-fg97](https://github.com/advisories/GHSA-qgw5-7j4f-fg97)): workflows run unsigned by default and the workflow loader executed referenced `file:` templates without the `-file` flag check the main loader applies → local file read from an untrusted workflow.
- **MySQL client LFA sandbox bypass** (CVE-2026-76803, [GHSA-xhmx-w2j4-rw3q](https://github.com/advisories/GHSA-xhmx-w2j4-rw3q)): a `javascript:` template (which runs **without** `-code` and unsigned by default) could connect with `allowAllFiles=true` in the DSN, letting a malicious MySQL server `LOAD DATA LOCAL INFILE` **any scanner-host path**. Generalized rule for every recon tool with DB protocol clients: the client-side LOCAL-INFILE capability is a file-read channel controlled by the *remote endpoint* — block or pin `allowLocalFiles/allowAllFiles` unless explicitly needed. This is the MySQL twin of the FTP/SMTP channel classes already cataloged here.
- **Env-var disclosure via double evaluation** (CVE-2026-76805, [GHSA-jpvm-9frm-hjcq](https://github.com/advisories/GHSA-jpvm-9frm-hjcq), incomplete fix of CVE-2026-41645): in DAST/fuzz mode, response-derived content captured by an `internal: true` extractor and reused in a later fuzz step was **re-substituted as template syntax** — with `-ev` enabled, a malicious target's response body `{{env_var}}` resolved to the scanner's environment. Operator-side finding class for any templating DSL: single-pass rendering fixes one call site; sweep every caller that re-evaluates substituted output.
- **Goja engine native-code execution** (CVE-2026-76819, [GHSA-vxg7-f2jj-jmqm](https://github.com/advisories/GHSA-vxg7-f2jj-jmqm), high): OOB heap write in the JS engine used for `javascript:` templates → arbitrary native code during a normal scan, reachable even from the template `init` section before security checks. v3.11.0 added JS-template signing as defense in depth. Lesson generalizes across all embedded-interpreter sandboxes (the n8n-class grammar rule cataloged on the Sept 15 MCP page): the sandbox's *engine* is attack surface, not just its API surface.

**Engagement takeaway:** if you run Nuclei with `-dast`, workflows, or third-party templates of any provenance, treat every template as code execution on your host (upgrade to ≥3.11.0; the signing-gate history on this one tool — mtime cache bypass above, DAST-branch bypass, JS bypass — shows the gate is patched per-incident, not per-design). As a target-side vector, attacker-controlled endpoints that bounce you into a scanner's `javascript:`/MySQL/fuzz paths are a scanner-pwn channel worth testing only in owned labs.
