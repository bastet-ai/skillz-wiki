...
Source: hourly offensive-security scan, 2026-06-22. Primary entries: GitHub advisories [GHSA-w6j9-vw59-27wv](https://github.com/advisories/GHSA-w6j9-vw59-27wv) / CVE-2026-25119, [GHSA-ffm6-vvph-g5f5](https://github.com/advisories/GHSA-ffm6-vvph-g5f5) / CVE-2026-21887, [GHSA-4mvw-j8r9-xcgc](https://github.com/advisories/GHSA-4mvw-j8r9-xcgc) / CVE-2024-37155, [GHSA-g9fx-5r4h-pcw3](https://github.com/advisories/GHSA-g9fx-5r4h-pcw3) / CVE-2026-31978, [GHSA-rhgp-6wq6-9j67](https://github.com/advisories/GHSA-rhgp-6wq6-9j67) / CVE-2026-32315, [GHSA-5pm9-r2m8-rcmj](https://github.com/advisories/GHSA-5pm9-r2m8-rcmj) / CVE-2025-58048, [GHSA-r3cw-c95m-wfh9](https://github.com/advisories/GHSA-r3cw-c95m-wfh9) / CVE-2026-46488, [GHSA-x93q-x9pc-w5hw](https://github.com/advisories/GHSA-x93q-x9pc-w5hw) / CVE-2026-44585, [GHSA-rv89-wch8-c574](https://github.com/advisories/GHSA-rv89-wch8-c574) / CVE-2026-44584, [GHSA-7wwh-xcc3-9fcg](https://github.com/advisories/GHSA-7wwh-xcc3-9fcg) / CVE-2026-44583, and follow-up [GHSA-5q4q-834j-g8g4](https://github.com/advisories/GHSA-5q4q-834j-g8g4) / CVE-2026-47198.
...
| [GHSA-rw9q-97r9-8gvh](https://github.com/advisories/GHSA-rw9q-97r9-8gvh) / CVE-2026-55488 | motionEye media handlers | absolute paths supplied as media filenames bypassed the intended camera media root because `os.path.join()` discarded the configured root | Add absolute-path probes to traversal matrices; prove only with a synthetic marker readable by the lab process. |
| [GHSA-j67x-q29f-qcvv](https://github.com/advisories/GHSA-j67x-q29f-qcvv) / CVE-2026-55863 | motionEye action handler | camera actions could be triggered without the authentication decorator used by neighboring handlers | Treat action, PTZ, snapshot, recording, and webhook helper routes as capability endpoints; prove with harmless lab-only action state changes. |
| [GHSA-phv5-334h-mxcw](https://github.com/advisories/GHSA-phv5-334h-mxcw) | motionEye default normal-user path plus config traversal | empty `@normal_password` plus config-file read exposed the admin hash, and the hash could act as API signing material | Default viewer-access deployments should be tested for hash-as-secret admin escalation with disposable hashes only; no cracking or real camera access. |
| [GHSA-qxvg-h7q2-hcxh](https://github.com/advisories/GHSA-qxvg-h7q2-hcxh) | motionEye chained LFI, hash-signature auth, restore extraction, and unauthenticated actions | local camera media reads, pass-the-hash admin signing, unsafe restore tar extraction, and action execution can combine into lab RCE | Chain validation belongs only in isolated labs; production evidence should stop at canary boundary proofs unless explicit exploit-chain authorization exists. |
...
| [GHSA-5q4q-834j-g8g4](https://github.com/advisories/GHSA-5q4q-834j-g8g4) / CVE-2026-47198 | Paymenter checkout Livewire component | URL-writable `config` parameters could inject undefined checkout keys that were stored as service properties and later passed into server provisioning | Customer-facing checkout flows need parameter-shadowing tests where user-supplied values compete with admin-defined hosting plan limits; prove only with disposable services and harmless quota/resource markers. |
| [GHSA-pgcq-8grm-5rx9](https://github.com/advisories/GHSA-pgcq-8grm-5rx9) / CVE-2026-55219 | Paymenter credit invoice payment | `lockForUpdate()` was called outside a database transaction, so concurrent `payWithCredit()` requests could reuse the same credit balance | Customer-credit flows need bounded concurrency testing around row-lock expectations; prove only with disposable invoices and test credits, never real balances or billable services. |
...
- Compare download/content routes and preview/thumbnail routes with the same encoded traversal filename matrix, including `%2F`-encoded separators and absolute-path variants, without targeting real files.
...
- For action endpoints, use only harmless actions against a disposable camera profile: status-only checks, snapshot markers, or synthetic action-script markers that write under a temp directory. Do not move real PTZ cameras, start/stop production recording, trigger alarms, or run host commands.
- For restore or chain testing, use a fully isolated motionEye lab with synthetic archives and config files. Evidence should show path/auth/control-flow reachability and temp markers only; never upload web shells, read real config, crack hashes, or alter production camera settings.
- Negative controls: canonical path containment before every open/delete/preview path, rejection of absolute paths before `os.path.join()`, authentication decorators on every action/control route, config restore path sanitization, signatures derived from non-reusable secrets instead of stored password hashes, and `0600` config permissions owned by the service user.
...
### Paymenter checkout configuration injection harness

- Preconditions: Paymenter lab or explicitly scoped staging tenant, disposable customer account, non-billable product, and a server extension whose provisioning parameters can be inspected without creating production resources.
- Start with a normal checkout URL and record the expected `config` keys emitted by the extension's `getCheckoutConfig()` method.
- Add undefined URL/query keys under the Livewire `config` alias that should not be customer-controlled, using inert values such as `skillz_plan_marker`, low-risk quota labels, or a test package identifier.
- Complete only a disposable checkout flow and inspect the resulting service properties and provisioning request in the lab.
- Positive evidence is an undefined user-supplied key stored on the service or prioritized over an administrator-defined extension setting during provisioning.
- Negative controls: strict allowlists for URL-writable checkout keys, validation and discard of undefined config entries before persistence, server-side plan/resource derivation during provisioning, and audit logs that distinguish customer-selected options from administrator policy.

### Paymenter credit race harness

- Preconditions: isolated Paymenter lab or explicitly scoped staging tenant, disposable customer, fake credit balance, and at least two non-billable invoices whose combined total exceeds the disposable credit.
- Establish the single-request baseline: one invoice paid with credit should reduce the balance and leave insufficient credit for the second invoice.
- Send two payment requests concurrently against separate invoices using the same customer session. Keep concurrency low and bounded, for example two to five synchronized requests, and stop as soon as the boundary is proven.
- Positive evidence is a before/after ledger showing the same starting credit satisfied more invoice value than should be possible, paired with request timestamps and invoice IDs from the lab.
- Do not test against production credits, real payment methods, live provisioning, or customer invoices. If the application triggers provisioning on paid invoices, use a fake product/provider that records an inert canary only.
- Negative controls: the lock is inside one database transaction, balance read and write are atomic, idempotency keys prevent duplicate settlement, and invoice state transitions are conditional on the current unpaid state.

## Reporting notes

- State the crossed boundary precisely: **client header to trusted reverse-proxy identity**, **ingestion URL to server-side request**, **GraphQL query text to schema-introspection policy**, **media preview path to outside-root file read**, **local config/hash to cookie-authenticated session**, **support attachment to executable public storage**, **customer service ID to support-ticket reference**, **old email verification to new address**, **webhook certificate URL to server-side fetch**, **checkout URL parameter to privileged provisioning property**, or **concurrent credit payment to non-atomic invoice settlement**.
