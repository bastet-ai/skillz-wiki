...
- **NocoBase SQL validation bypasses** — [GHSA-wrwh-c28m-9jjh](https://github.com/advisories/GHSA-wrwh-c28m-9jjh), [GHSA-4948-f92q-f432](https://github.com/advisories/GHSA-4948-f92q-f432), and [GHSA-vx58-fwwq-5g8j](https://github.com/advisories/GHSA-vx58-fwwq-5g8j) / CVE-2026-34825: `@nocobase/plugin-collection-sql < 2.0.39` missed `checkSQL`, `@nocobase/database < 2.0.39` had recursive eager-loading SQLi through string concatenation, and `@nocobase/plugin-workflow-sql <= 2.0.29` allowed SQL injection through workflow SQL-node template-variable substitution. Patch workflow SQL to **2.0.30+** and the collection/database fixes to **2.0.39+**.
...
2. Patch NocoBase collection/database packages to `2.0.39+`, `@nocobase/plugin-workflow-sql` to `2.0.30+`, MikroORM packages past the fixed release, and `mathjs` to `15.2.0+`.
3. Search for APIs exposing XML node construction, custom SQL fragments, workflow SQL nodes, sort/filter/include fields, JSON-path keys, formula expressions, or object mutation to end users.
4. Add regression tests with recursive XML trees, malicious comments/processing instructions, SQL identifier metacharacters, workflow template variables that expand to SQL syntax, JSON-path escapes, and prototype-ish attribute names.

## Durable controls

- Represent query structure with typed builders and enum allowlists; never concatenate identifiers, operators, or clauses from request fields.
- Treat workflow/template substitutions as values only; if a variable can alter SQL syntax, move it behind a parameter binder or remove the feature.
