...
- **Froxlor FTP shell assignment, SSH key sync, and DNS renderer boundaries** — Froxlor `2.3.6` lets shell-enabled customer accounts submit shells outside `system.available_shells` and can later append customer-supplied SSH keys through a root-owned sync path that follows `~/.ssh/authorized_keys` symlinks. The DNS record validation follow-up was updated on 2026-06-29 with LOC newline-preservation and TLSA length-bound issues that reinforce the zone-file injection workflow now expanded on the dedicated Froxlor DNS page.
