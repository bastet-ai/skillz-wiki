...
## September 22 follow-up: Kerberos federation provider accepts an unauthenticated KDC (CVE-2026-95503)

[GHSA-35j3-ccrv-pgvj](https://github.com/advisories/GHSA-35j3-ccrv-pgvj) / CVE-2026-95503: when Keycloak's **Kerberos federation provider** performs password authentication **without SPNEGO**, it never requests a server ticket to verify the KDC's identity — an attacker positioned on the network path to the KDC can **spoof the KDC and pass arbitrary credential checks**, authenticating as any federated Kerberos user.

- **Axis: identity assertions from unauthenticated remote services must be bound by cryptography the client demands.** The client (Keycloak) trusted a network-located service's "yes" with no ticket-of-authentication handshake. Sibling of the Sept 21 ADSys trust-fetch item (trust material over unauthenticated channels) and the pre-SPNEGO FAST/preauth family: the check to run on any Kerberos/LDAP/RADIUS federation integration is *does the verifier prove the responder's identity, or only that the responder answered?*
- **Operator recon shape:** fingerprint Keycloak realms with Kerberos federation enabled (login page offering Kerberos/SPNEGO-backed realms), determine whether the KDC is reachable on the same segment as the Keycloak node (same-LAN/VLAN positions are the precondition), and note that this is a network-position attack, not a remote web attack — report it as perimeter-to-identity bridging guidance for authorized internal assessments.
- **Lab-only proof:** lab realm + Kerberos federation without SPNEGO, a fake KDC you bind on a lab host between the two (or point Keycloak's KDC config at your fake service), and show a wrong password accepted by the fake KDC yields a Keycloak session for the synthetic user. Never test against a production KDC or real user credentials.

## September 23 follow-up: PAR request-uri reuse under `prompt=none`, and header-trusted Conditional OTP skip (2 GHSAs)

- **`GHSA-j24v-wv3r-fjch` / [CVE-2026-96446](https://nvd.nist.gov/vuln/detail/CVE-2026-96446) — Pushed Authorization Requests: single-use enforcement of `request_uri` is skipped on the silent-authentication path.** With `prompt=none`, the flow short-circuits steps of the authorize process, and the "pushed request URI is consumed once" rule is one of the skipped steps → the **same `request_uri` mints multiple authorization codes** for an already-signed-in user (FAPI-2 violation). Operator shape: any OIDC feature that "skips steps when already authenticated" (`prompt=none`, silent SSO, session-refresh paths) is a **step-skipping surface** — enumerate which validations each fast-path bypasses (single-use consumption, consent, ACR/LoA checks, state binding) by replaying one pre-created artifact (PAR request, auth request) twice through the silent path with two different downstream `state`s. This page's Keycloak saga now spans verb families, lifecycle state, cache scope, KDC trust, and now *silent-path step-skipping* — the sweep is per-flow-path, not per-endpoint.
- **`GHSA-jj9w-64w4-2wcx` / [CVE-2026-96445](https://github.com/advisories/GHSA-jj9w-64w4-2wcx) — Conditional OTP authenticator trusts untrusted HTTP headers to decide OTP skip.** The conditional authenticator evaluates request headers to decide whether to skip the second factor without verifying they came from a trusted proxy. Password-holder + crafted header = 2FA bypass. Canonical rule (9router `X-9r-Real-Ip`, OpenClaw chat identities, Tomcat realm first-wins): **step-up/conditional-auth decisions sourced from request headers must have a trusted-peer check; enumerate every conditional authenticator's trigger header and replay it from an untrusted peer.** Lab proof: lab realm with Conditional OTP + an owning test user, send the skip-trigger header from a non-proxy client and record that the OTP step is absent from the required-actions list.
- Validation boundary: lab realms, synthetic users, your own IdP; never replay a real user's `request_uri` or bypass 2FA on a live identity provider.
