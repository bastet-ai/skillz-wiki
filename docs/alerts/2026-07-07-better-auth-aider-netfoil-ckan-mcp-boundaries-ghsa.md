...
## September 22 evening follow-up: IPv6-transition wrappers bypass IPv4 denylists (Cloudreve + LightRAG, 2 GHSAs)

Two same-wave advisories prove the same guard gap in two unrelated projects: an SSRF classifier that checks resolved IPs with `IsLoopback/IsPrivate/IsLinkLocalUnicast/...`-style builtins **never decodes IPv4-in-IPv6 transition wrappers**, so the classifier sees "ordinary global IPv6" while the network delivers to the embedded IPv4:

- **Cloudreve < 2026-07-15 remote-download workflow** (CVE-2026-79913, [GHSA-jvh5-97xg-v99f](https://github.com/advisories/GHSA-jvh5-97xg-v99f)): `checkIP` accepts NAT64 well-known-prefix (`64:ff9b::127.0.0.1`, RFC 6052), IPv4-compatible (`::127.0.0.1`, RFC 4291), and 6to4 (`2002:7f00:0001::`, RFC 3056) forms — attacker-controlled AAAA record (or, on a DNS64/NAT64 network, *any* hostname) reaches loopback, RFC1918, or `169.254.169.254`.
- **LightRAG native-markdown image-download guard** (CVE-2026-85740, [GHSA-vv3m-f8x4-7377](https://github.com/advisories/GHSA-vv3m-f8x4-7377), high): identical wrapper bypass on the image-fetch leg of the RAG ingestion pipeline — the AI-workflow ingestion channel is an SSRF channel.

Standing battery update for every URL guard tested from this page's string-vs-resolution table: after the DNS-A-record leg, sweep the **transition-wrapper spellings** of the denied IPv4 — `64:ff9b::<v4>`, `::<v4>`, `2002:<hex(v4)>::`, plus IPv4-mapped `::ffff:<v4>` — against both the validator's classifier and an owned canary listener. The finding class: **IP-classification builtins inspect the outer address only; the routing layer decodes.** Corollary for report language: on networks with DNS64 (mobile carriers, some clouds), *every* hostname can be made to resolve into a wrapper — the guard bypass needs no DNS control at all, only an AAAA-capable resolver path. Lab-bounded as always: owned domains, loopback-bound canary listener, never real IMDS.
