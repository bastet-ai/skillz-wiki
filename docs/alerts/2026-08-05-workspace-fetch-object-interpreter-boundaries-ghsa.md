...
### Tauri HTTP plugin follow-up: scope is checked once, redirects re-resolve freely (Sept 22)

Tauri's HTTP plugin ([GHSA-5gm7-pf8v-vxwc](https://github.com/advisories/GHSA-5gm7-pf8v-vxwc) / [CVE-2026-95623](https://nvd.nist.gov/vuln/detail/CVE-2026-95623), fixed http-v2.7.0) validates the requested URL against the app's configured scope allowlist **only on the initial request**; reqwest then follows 3xx responses internally without re-checking each hop. An attacker who controls any allowed URL — or finds an open redirect on an allowed host — reaches disallowed destinations (localhost services, cloud metadata, internal hosts) through a client that appears scoped. Two durable axes:

- **Allowlist scope is a property of the first URL, not the fetch.** For every scope/policy engine around an HTTP client (Tauri scope, proxy allowlists, CSP-adjacent fetch wrappers, plugin `connect-src` analogues), ask whether the check runs per hop or per invocation, and whether the client library follows redirects internally (`redirect::Policy` defaults, `follow_redirects` flags) outside the policy layer's view. The desktop-app case is sharpest because the scope is the *only* network sandbox for the webview-facing HTTP API.
- **The precondition is an allowed-host open redirect.** On desktop-app assessments, enumerate allowed hosts and test each for an open redirect; one `302` from an allowlisted domain converts a scoped client into an unscoped one. Prove with an owned redirector on an owned "allowed" origin targeting an owned listener returning only `INTERNAL-CANARY-<uuid>`, and record policy-decision URL vs final socket peer per hop. Never target real metadata endpoints or user-installed app scopes.

### September 22 evening follow-up: Tauri updater anti-rollback is frontend-controllable + sandbox secrets in world-readable argv

- **Tauri updater plugin `check` IPC takes `allowDowngrades` from the webview** (CVE-2026-95624, [GHSA-wqjv-7vqv-x3mj](https://github.com/advisories/GHSA-wqjv-7vqv-x3mj), 6.8): the `check` command accepts an `allowDowngrades: true` boolean **directly from frontend JavaScript**, swapping the version comparator from "update must be newer" to "update must be *different*" — and the default permission set grants `allow-check` to the webview, so **any XSS in the app frontend disables the only anti-rollback protection the updater has**, enabling downgrades onto a vulnerable prior version without faking a version number. Desktop-app audit rule: enumerate IPC commands whose *parameters* flip a security policy (not just data), and check the default permission capability list for which commands the webview may call — an XSS becomes an update-downgrade bridge when a policy verdict rides in from untrusted JS. Pair with the CVE-2026-95623 hop-scope item above: same plugin family, two ways a webview compromise exits the sandbox.
- **microsandbox secrets in `--network-config <json>` argv** (CVE-2026-61670, [GHSA-m8f5-rh7h-vgg3](https://github.com/advisories/GHSA-m8f5-rh7h-vgg3), 6.5): the SDK serializes the full network config *including real secret values* onto the `msb sandbox` child's command line (plus `--env KEY=VALUE` args), and Linux exposes `/proc/<pid>/cmdline` world-readable (`ps` on macOS) — any local unprivileged process reads the host-side secrets for the sandbox's lifetime, defeating the "secrets can't leak" host-boundary guarantee. Reusable local-recon primitive: on any multi-user host, `cat /proc/*/cmdline` and `ps eww` are a credential harvest — grep for `--network-config`, `--env`, `--token`, `--password`, JSON-shaped args. Sandbox/agent platforms that pass config to helper processes via argv inherit this; prove only on your own host with your own synthetic marker secret.
...
### Home Assistant follow-up: mDNS announcements are an unauthenticated fetch-trigger channel on the LAN (Sept 22 sync)

- **Home Assistant Green < 2026.2.3 `_ipp._tcp` zeroconf SSRF** (CVE-2026-91129, [GHSA-4ghv-53cq-7wp3](https://github.com/advisories/GHSA-4ghv-53cq-7wp3), medium): any LAN peer can send a crafted mDNS service announcement; the IPP integration's `async_step_zeroconf` handler extracts `host`, `port`, and `base_path` **straight from the announcement with no validation** and the integration then makes HTTP requests there — following redirects from the attacker-controlled host, reaching internal services incl. localhost-bound ones, with **zero user interaction or authentication**. New fetch-leg category for this page's outbound-fetch inventory: **multicast service-discovery protocols (mDNS/LLMNR/NBNS/SSDP/SOAP discovery) are unauthenticated input channels on any link-local network** — the "attacker URL" is simply a packet on the wire. Bug-hunt checklist for IoT/home-automation/smart-device targets that aren't exposed remotely: on the same L2/link, forge a discovery announcement for each auto-integrating service type (`_ipp._tcp`, `_hap._tcp`, `_airplay._tcp`, `_googlecast._tcp`, SSDP `ssdp:discover`) and observe whether the device connects out to your owned callback; the redirect-following leg turns one announcement into a full SSRF chain. Red-team LAN-adjacent recon: these announcements are also a zero-credential beacon/verification channel. Prove only against owned/lab devices with an owned callback host.
