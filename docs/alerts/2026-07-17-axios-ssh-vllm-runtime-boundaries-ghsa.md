...
The July 20 Axios follow-ups add [GHSA-xj6q-8x83-jv6g](https://github.com/advisories/GHSA-xj6q-8x83-jv6g), [GHSA-gcfj-64vw-6mp9](https://github.com/advisories/GHSA-gcfj-64vw-6mp9), [GHSA-7q8q-rj6j-mhjq](https://github.com/advisories/GHSA-7q8q-rj6j-mhjq), [GHSA-mmx7-hfxf-jppx](https://github.com/advisories/GHSA-mmx7-hfxf-jppx), and [GHSA-f4gw-2p7v-4548](https://github.com/advisories/GHSA-f4gw-2p7v-4548). They extend the same theme from inherited Basic-auth subfields to interceptor-cloned proxy config, nested serializer values, bodies on bodyless aliases, and `0.0.0.0` proxy routing. The adjacent serializer recursion and streamed-upload limit records are tracked only as availability/policy-limit context.
...
| [GHSA-gcfj-64vw-6mp9](https://github.com/advisories/GHSA-gcfj-64vw-6mp9) | Axios Node HTTP adapter with an interceptor returning a normal cloned config | A separately polluted inherited `proxy` value becomes visible again after `{...config}`/`Object.assign` discards Axios's null-prototype hardening | Test plaintext requests through a fake proxy and distinguish high-level interceptor reachability from direct adapter use. |
| [GHSA-7q8q-rj6j-mhjq](https://github.com/advisories/GHSA-7q8q-rj6j-mhjq) / [GHSA-mmx7-hfxf-jppx](https://github.com/advisories/GHSA-mmx7-hfxf-jppx) | Axios nested options and bodyless aliases | Inherited auth/serializer values or `data` can alter request construction after a separate pollution primitive | Compare own versus inherited fields at a loopback capture service; claim only the observed header, query, or body change. |
| [GHSA-f4gw-2p7v-4548](https://github.com/advisories/GHSA-f4gw-2p7v-4548) | Axios Node environment-proxy selection | `0.0.0.0` may miss local `NO_PROXY` intent and reach the configured proxy | Add unspecified-address forms to local-service proxy-routing matrices with an owned local canary. |
...
For the new proxy/body/serializer variants, reuse the disposable process but add an interceptor that returns `{...config}`, fake inherited `proxy`, `paramsSerializer`, and `data` values, and a loopback origin plus fake HTTP proxy. Test each property in a fresh process and clean `Object.prototype` in `finally`. Positive evidence must identify the exact gadget: interceptor clone routes a plaintext request through the fake proxy; nested serializer changes only the synthetic query; or a bodyless alias emits the marker body. For `0.0.0.0`, compare `localhost`, `127.0.0.1`, `[::1]`, and `0.0.0.0` under the same `NO_PROXY` and redirect configuration. Never place real authorization headers or request bodies in the harness.
