...
| [GHSA-489g-7rxv-6c8q](https://github.com/advisories/GHSA-489g-7rxv-6c8q) | `mcp-atlassian` `X-Atlassian-{Jira,Confluence}-Url` handling | middleware validates one DNS answer, but the fetcher later connects to the raw hostname without IP pinning | Add DNS-rebinding TOCTOU checks to MCP SSRF assessments: prove validate-time IP and connect-time IP can diverge using owned DNS/canaries only. |
...
### MCP Atlassian DNS-rebinding TOCTOU checks

- Treat URL SSRF patches as incomplete until the connection is bound to the same address family and IP that the guard validated. The `mcp-atlassian` advisory is the reusable pattern: `validate_url_for_ssrf()` resolves a caller-controlled `X-Atlassian-Jira-Url` or `X-Atlassian-Confluence-Url` hostname once, returns only a pass/fail verdict, and later fetcher construction reuses the raw hostname, allowing a second DNS lookup to reach a different address.
- In an authorized lab, instrument the exact URL-validation and outbound-fetch path. Capture a compact table: raw header URL, guard-time `getaddrinfo()` answer, guard decision, connect-time `getaddrinfo()` answer, final socket peer or owned callback, and whether the implementation pins the validated IP.
- Use owned rebinding domains and synthetic callback/private canaries. A safe proof can be a structural harness that forces first lookup to a public owned IP and second lookup to a lab canary IP; it does not need cloud metadata or real internal service access.
- Add a negative control where the guard sees the blocked address at validation time and rejects it, then a patched control where the fetcher connects only to the validated address or uses a resolver/cache bound to the validation result.
- Report the boundary as **header-controlled URL -> validate-time DNS decision -> unpinned connect-time DNS re-resolution -> SSRF guard bypass**. Do not query metadata services, customer Jira/Confluence instances, internal admin panels, or production private networks.
...
- Name the exact crossed boundary: **agent session ID to authorization context**, **manifest URL to server fetch**, **git-revision spec to ignored external-ref deny**, **tenant scaler field to libpq key injection**, **foreign WebAuthn credential to current-session freshness**, **body row ID to cross-namespace deployment action**, **secret-path MCP UI to bare-root route**, **MCP file-reader URL to server-side fetch**, **validate-time DNS answer to unpinned connect-time hostname**, **MCP search regex to shared runtime resource consumption**, **email address to SMTP command line**, **deep link to workspace command execution**, or **foreign flow ID to response execution**.
