...
## September 21 follow-up: enriched RHOAI/DSPO details — native-role aggregation, PodTemplateSpec passthrough, and weak credential derivation

The September 21 advisory-enrichment wave added full descriptions to several previously terse OpenShift AI records, sharpening three reusable axes on this page:

- **Native Kubernetes roles as a privilege backdoor** — [GHSA-v6pg-cp3x-x49r / CVE-2026-18982](https://github.com/advisories/GHSA-v6pg-cp3x-x49r) and [GHSA-h7xh-qc2m-rc58 / CVE-2026-18951](https://github.com/advisories/GHSA-h7xh-qc2m-rc58): the training-operator's aggregated ClusterRules land on the **stock `edit` and `admin` ClusterRoles**, so *any* namespace holder of the vanilla roles can create/modify/delete `TrainJobs` — and unrestricted `PodTemplateSpec` passthrough on those jobs adds service-account impersonation, host-filesystem access, and code execution. The audit: enumerate CRDs reachable through aggregated rules on roles you already hold (`kubectl auth can-i --list` under a plain-edit test user, then grep for CRD groups), and treat every `PodTemplateSpec`/`template` field on a job-like CRD as a pod-creation sink regardless of which controller renders it. This is the same caller-selected-identity family as the DSPO `setDefaultServiceAccount` record already on this page ([GHSA-g68c-wmxr-x3rh](https://github.com/advisories/GHSA-g68c-wmxr-x3rh)) — the chain is: stock namespace role → job CR with foreign SA + hostPath template → controller renders under its own elevated authority.
- **Weak PRNG in operator-generated credentials** — [GHSA-5qmg-pgg7-3rjj / CVE-2026-18611](https://github.com/advisories/GHSA-5qmg-pgg7-3rjj): the DSPO operator derives MariaDB root/user passwords and MinIO access/secret keys from a **cryptographically weak PRNG**, so anyone who can reach the MinIO Route or MariaDB Service can *derive* them offline. Operator check: locate any backing-store Route/Service the operator exposes, and in an authorized lab reproduce derivation from the operator image's seeding logic rather than brute-forcing; a bounded positive is **a recomputed credential matching a synthetic seed value** — report the reachability surface and derivation input, never live passwords. Pair with this page's token-entropy siblings on the September 18 page.
- **Operator ClusterRole over-scoping as post-compromise blast radius** — [GHSA-vh9j-mvrr-922q / CVE-2026-18608](https://github.com/advisories/GHSA-vh9j-mvrr-922q): DSPO's own ClusterRole carries pod-exec and cluster-wide role management, so any DSPO pod foothold (the DSN/local-infile leg already on this page) converts to cluster admin. Recon value: dump operator ClusterRoles (`kubectl get clusterrole -o yaml | grep -B5 pods/exec`) — over-scoped operators are the privilege ladder *after* the first boundary break, and the enumeration itself needs only a low-priv API token.

All three remain Red Hat-enriched GitHub mirrors; confirm distribution, operator version, and enabled component against the primary advisory before reporting. Proofs stay in disposable clusters with synthetic roles, fake service accounts, inert job templates stopped at denied pod-create sinks, and no live credential derivation.

## Evidence and reporting checklist

- [ ] Aggregation findings state which stock role grants which CRD verb and prove it with a canary identity that holds nothing else.
- [ ] PodTemplateSpec proofs stop at a denied pod-spec recorder; no privileged or hostPath pod is ever scheduled.
- [ ] Weak-PRNG proofs use synthetic seeds; no live database or object-store credential appears in evidence.
