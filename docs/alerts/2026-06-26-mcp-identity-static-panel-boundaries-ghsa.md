...
| [GHSA-4hf8-5mjm-rfgq](https://github.com/advisories/GHSA-4hf8-5mjm-rfgq) / CVE-2026-49357 | `line-desktop-mcp` | streamable HTTP mode exposed LINE Desktop read/send tools without MCP authentication | Treat local desktop MCP bridges as account-capable control planes; verify bind address, auth on list/call routes, and read/send separation with owned accounts only. |
...
- For each exposed tool, compare session-level scope with tool-level authorization: low-privilege sessions against write/delete/spend tools, unauthenticated requests against list/call routes, browser/LAN clients against local desktop bridges, and URL-fetch tools against owned callback URLs.
