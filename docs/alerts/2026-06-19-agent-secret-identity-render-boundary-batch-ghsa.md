...
June 22 update: two adjacent Network-AI advisories extend the agent-control pattern. [GHSA-48x2-6pr9-2jjf](https://github.com/advisories/GHSA-48x2-6pr9-2jjf) shows `EnvironmentManager.restore(env, backupId)` accepting traversal-style backup IDs and copying arbitrary directories into an environment data directory through the default CLI restore path. [GHSA-mxjx-28vx-xjjj](https://github.com/advisories/GHSA-mxjx-28vx-xjjj) shows the exported ApprovalInbox HTTP server exposing unauthenticated approval and denial routes with wildcard CORS, so local, SSRF-reachable, non-loopback, or browser-origin clients could approve pending high-risk agent actions without the operator's consent.
...
| GHSA-48x2-6pr9-2jjf | Network-AI `EnvironmentManager.restore()` | caller-controlled backup IDs could traverse out of `.backups` and copy arbitrary source directories into environment data | Treat backup restore IDs as path selectors; validate canonical containment before collection/copy and prove only with disposable source directories containing marker files. |
| GHSA-mxjx-28vx-xjjj | Network-AI `ApprovalInbox` HTTP server | unauthenticated approval/denial routes plus wildcard CORS could defeat human-in-the-loop gates for high-risk actions | Agent approval surfaces need bind-address, auth, `Origin`, and state-changing route checks; prove with inert pending actions and never approve real shell, write, or spend operations. |
...
- For approval inboxes or review UIs, create one inert pending action, then test unauthenticated direct requests, loopback-only browser requests with `Origin` set to an owned page, and same-host/SSRF-style reachability. Positive evidence is limited to the pending action changing state, not execution of a dangerous command.
- For backup restores, create a fake external backup directory with marker-only files and attempt traversal-style backup IDs; positive evidence is the marker being copied into a disposable environment data directory.
