...
- [Realtime view-framework state-binding boundaries: djust WebSocket `update_model` mass-assignment of any public view attribute (client-supplied field name, default allow-all), DEBUG-gated observability/eval endpoints with an opt-in localhost middleware the docs omit, WS/SSE-only tenant-isolation fail-open + `csrf_exempt` SSE CSRF follow-ups, OpenTelemetry.Resources.Host macOS bare-name PATH hijack → local LPE (5 GHSAs)](alerts/2026-09-16-djust-websocket-mass-assignment-debug-gated-observability-and-otel-path-hijack-ghsa.md)
